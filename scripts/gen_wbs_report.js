...
function getOutputPath(argv) {
  ...
  if (outputIndex >= 0) {
    if (!argv[outputIndex + 1]) throw new Error("--output requires a file path");
    return path.resolve(argv[outputIndex + 1]);
  }
  return path.resolve(__dirname, "..", "artifacts", "pmo", "wbs", "信息化项目WBS优化调整报告_V1.0.docx");
}

const outPath = getOutputPath(process.argv.slice(2));
