...
const tmp = mkdtempSync(join(tmpdir(), 'reviewItem-sankey-preview-'));

try {
  const runDir = join(tmp, 'review-run');
  ...
  const defaultResult = spawnSync(
    process.execPath,
    [
      resolve(ROOT, 'scripts', 'build-reviewItem-sankey-preview.mjs'),
      '--review-run',
      runDir,
    ],
  ...
  assert.equal(existsSync(defaultOut), true, 'default preview HTML should be written next to the review run');
  assert.match(defaultResult.stdout, /artifacts|review-run|preview\.html/);
  ...
  assert.match(html, /待确认L3/);
  assert.match(html, /待确认A1/);
  ...
  assert.ok(graph.nodes.length <= 12, 'overview Sankey should aggregate by issue type instead of rendering every reviewItem as a leaf');
  assert.equal(
    graph.nodes.some(node => node.name.includes('工程数据集审签和发放管理标准') && node.name.includes('第14页')),
    false,
    'individual reviewItem details belong in the table, not the overview Sankey nodes',
  );
} finally {
