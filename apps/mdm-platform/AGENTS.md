...
- 流程治理前端只保留流程编制、跨部门承接待办和承接冲突待办三个工作区。完整`process-governance-v3` JSON是编制内容真源，承接和待办表是治理投影；草稿保存必须校验`expected_revision`。v1、v2只作为兼容输入，表单状态统一迁移为`unspecified`，不得推断现状或拟设计状态。
- 3001继续独立运行，MDM兼容导入用户选择的v1、v2和v3文件，并统一保存、导出v3。MDM改造不得停止、重绑或代管3001。
