...
assert.doesNotMatch(publishSource, /queueProcessDataGovernanceCreationTask/, 'publishing must not create governance work packages automatically');
const routeSource = fs.readFileSync(path.join(__dirname, '..', 'server', 'routes', 'processDataGovernance.js'), 'utf8');
assert.match(routeSource, /router.post\('\/creation-tasks\/reconcile'/);
assert.match(routeSource, /repo.queueAndMaterialize\(processVersionId, actor\)/);
