...
const { inspectAnalysisIssues, applyAnalysisIssues } = require('../server/analysisIssueMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyAnalysisIssues(db) : inspectAnalysisIssues(db), 120000);
