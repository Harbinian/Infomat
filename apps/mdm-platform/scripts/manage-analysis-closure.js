...
const { inspectAnalysisClosure, applyAnalysisClosure } = require('../server/analysisClosureMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyAnalysisClosure(db) : inspectAnalysisClosure(db), 120000);
