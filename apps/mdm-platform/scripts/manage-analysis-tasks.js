...
const { inspectAnalysisTasks, applyAnalysisTasks } = require('../server/analysisTaskMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyAnalysisTasks(db) : inspectAnalysisTasks(db), 120000);
