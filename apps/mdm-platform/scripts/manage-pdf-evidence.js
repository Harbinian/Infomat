...
const { inspectPdfEvidence, applyPdfEvidence } = require('../server/pdfEvidenceMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyPdfEvidence(db) : inspectPdfEvidence(db), 120000);
