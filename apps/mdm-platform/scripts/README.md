...
| `npm run test:identity-mysql` | 身份/RBAC MySQL 模型：repository 契约、登录、会话、本人改密、管理员用户/权限读写路由验证 | 使用 fake MySQL pool 和 fake repository，不连接真实库；默认不开启该切换 |
...
| `test-identity-mysql-repository.js` | 验证身份/RBAC MySQL repository 可读取用户、部门、登录凭据、角色、继承权限、字段约束、本人改密状态、管理员用户列表、权限清单和管理员用户写入 | 使用 fake MySQL pool，不连接真实库 |
| `test-org-me-mysql-api.js` | 验证 `MDM_IDENTITY_READ_MODEL=mysql` 时登录、`/api/org/me`、`/api/org/session`、本人密码状态、本人改密、管理员用户/权限读写接口走 MySQL repository | 使用 fake repository，不连接真实库 |
