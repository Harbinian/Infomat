...
| `npm run test:identity-mysql` | 身份/RBAC MySQL 读取模型：repository 契约、`/api/org/me` 和 `/api/org/session` 注入式路由验证 | 使用 fake MySQL pool 和 fake repository，不连接真实库；默认不开启该切换 |
...
| `test-identity-mysql-repository.js` | 验证身份/RBAC MySQL repository 可读取用户、部门、角色、继承权限和字段约束 | 使用 fake MySQL pool，只读仓库 |
| `test-org-me-mysql-api.js` | 验证 `MDM_IDENTITY_READ_MODEL=mysql` 时 `/api/org/me` 和 `/api/org/session` 读取 MySQL repository | 使用 fake repository，不连接真实库 |
