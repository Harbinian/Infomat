...
| `npm run test:process-design` | 文档结构化输出 API、MySQL schema、制度 profile、术语、流程明细、L1/L2 既有映射枚举校验、行为详情、跨部门承接回写、附表结构、自动编号、字段空格校验、Markdown 草案导出、发布链路，以及术语/流程/业务行为编辑、删除、作废和只读状态 | 使用 fake process-design repository 和 fake MySQL 身份 repository，不连接真实库 |
