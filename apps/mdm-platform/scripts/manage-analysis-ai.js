...
const { inspectAnalysisAi, applyAnalysisAi } = require('../server/analysisAiMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyAnalysisAi(db) : inspectAnalysisAi(db), 120000);
