...
async function main() {
  ...
  try {
    if (args.has('--dry-run')) {
      console.log(JSON.stringify({ mode: 'dry-run', database: redactMysqlConfig(config), ...(await inspectProcessDataGovernance(pool)) }, null, 2));
      return;
    }
    if (args.has('--apply')) {
      console.log(JSON.stringify({ mode: 'apply', database: redactMysqlConfig(config), ...(await applyProcessDataGovernance(pool)) }, null, 2));
      return;
    }
    if (args.has('--rollback')) {
      console.log(JSON.stringify({ mode: 'rollback', database: redactMysqlConfig(config), ...(await rollbackProcessDataGovernance(pool)) }, null, 2));
      return;
    }
