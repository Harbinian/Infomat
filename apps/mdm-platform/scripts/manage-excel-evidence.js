...
const { inspectExcelEvidence, applyExcelEvidence } = require('../server/excelEvidenceMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyExcelEvidence(db) : inspectExcelEvidence(db), 120000);
