...
const { inspectAnalysisRuns, applyAnalysisRuns } = require('../server/analysisRunMigration');
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  try {
    const result = await withMysqlDeadline(pool, db => mode === '--apply' ? applyAnalysisRuns(db) : inspectAnalysisRuns(db), 120000);
