...
async function applyPdfEvidence(db) {
  const [[lock]] = await db.execute("SELECT GET_LOCK('mdm_pdf_evidence_migration_v1',10) acquired");
  if (Number(lock.acquired) !== 1) throw failure('DEFINITION_ANALYSIS_MIGRATION_BUSY', 409);
  try {
    const before = await inspectPdfEvidence(db);
    ...
    const after = await inspectPdfEvidence(db);
    ...
    return inspectPdfEvidence(db);
  } finally { await db.execute("SELECT RELEASE_LOCK('mdm_pdf_evidence_migration_v1')"); }
}
module.exports = { inspectPdfEvidence, applyPdfEvidence, MIGRATION_KEY, tables };
