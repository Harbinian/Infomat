// Immutable Pdf snapshots reuse source batches. No ledger creation or inferred mappings.
const { id, json, digest, parse, lastId } = require('./dataMapDefinitionValues');
const { VERSION, parsePdfEvidence, locate, fail } = require('./pdfEvidenceParser');
const { MIGRATION_KEY } = require('./pdfEvidenceSchema');
...
async function load(db, who, batch, scope, resolve) {
  await ready(db);
  const [[row]] = await db.execute('SELECT snapshot_json,snapshot_digest FROM data_map_pdf_evidence WHERE batch_id=? FOR SHARE', [id(batch.batch_id)]);
...
module.exports = helpers => {
  ...
  return {
    async registerPdfEvidence(session, payload, bytes) {
      ...
      const parsed = await parsePdfEvidence(bytes,payload.original_name);
      ...
      return transaction(async db => {
        ...
        return request(db,who,'pdf_evidence',body,async()=>{
          ...
          await db.execute("INSERT INTO data_map_import_batches(source_type,file_name,status,note) VALUES ('pdf_evidence',?,'imported',?)",[value.original_name,'pdf-evidence-v1: evidence registered; no ledger import or business confirmation']);
          ...
          await db.execute('INSERT INTO data_map_pdf_evidence(batch_id,snapshot_json,snapshot_digest) VALUES (?,?,?)',[batchId,json(value),digest(value)]);
          return {batch_id:batchId,duplicate:false,raw_sha256:value.raw_sha256};
        });
      });
    },
    getPdfEvidence(session,batchId,query={}) { return transaction(async db=>{
      if(Object.keys(query).some(k=>!['anchor','offset','page'].includes(k)))throw fail('QUERY');
      const who=await actor(db,session),source=await resolve(db,who,'template',id(batchId));
      if(source.snapshot.source_kind!=='pdf_material')throw fail('NOT_FOUND',404);
      ...
      if(query.page!==undefined&&(!/^[1-9]\d{0,2}$/.test(String(query.page))||Number(query.page)>doc.pages.length))throw fail('QUERY');
      const items=query.page===undefined?doc.anchors:doc.anchors.filter(a=>a.page===Number(query.page));
      const offset=Number(query.offset||0),anchors=items.slice(offset,offset+100);
      return {source:source.snapshot,coverage:doc.coverage,pages:doc.pages,links:doc.links,anchors,next_offset:offset+100<items.length?offset+100:null};
    }); }
  };
};
