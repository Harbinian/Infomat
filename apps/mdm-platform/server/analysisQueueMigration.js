const { MIGRATION_KEY, tables, statements } = require('./analysisQueueSchema');
const dependencies = [require('./analysisRunSchema').MIGRATION_KEY];
const { compareCreateStatements } = require('./processV7M0Baseline');
const { failure } = require('./dataMapDefinitionValues');
async function inspectAnalysisQueue(db) {
  const missing = [], drift = [];
  ...
  const prerequisite = await require('./analysisRunMigration').inspectAnalysisRuns(db);
  if (!prerequisite.ready) drift.push({ dependency: 'analysis-runs-structure', missing: prerequisite.missing, drift: prerequisite.drift });
...
async function applyAnalysisQueue(db) {
  const [[lock]] = await db.execute("SELECT GET_LOCK('mdm_analysis_queue_migration_v1',10) acquired");
  if (Number(lock.acquired) !== 1) throw failure('DEFINITION_ANALYSIS_QUEUE_MIGRATION_BUSY', 409);
  try {
    const before = await inspectAnalysisQueue(db);
    if (before.drift.length) throw failure('DEFINITION_ANALYSIS_QUEUE_SCHEMA_DRIFT', 409);
    for (const sql of statements()) if (before.missing.includes(sql.match(/CREATE TABLE IF NOT EXISTS (\w+)/)[1])) await db.execute(sql);
    const after = await inspectAnalysisQueue(db);
    if (after.missing.length || after.drift.length) throw Object.assign(failure('DEFINITION_ANALYSIS_QUEUE_SCHEMA_DRIFT', 409), { inspection: after });
    await db.execute('INSERT IGNORE INTO schema_migrations(migration_key) VALUES (?)', [MIGRATION_KEY]);
    return inspectAnalysisQueue(db);
  } finally { await db.execute("SELECT RELEASE_LOCK('mdm_analysis_queue_migration_v1')"); }
}
module.exports = { inspectAnalysisQueue, applyAnalysisQueue, MIGRATION_KEY, tables };
