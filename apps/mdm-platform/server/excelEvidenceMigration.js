const { MIGRATION_KEY, tables, statements } = require('./excelEvidenceSchema');
const { failure } = require('./dataMapDefinitionValues');
const { compareCreateStatements } = require('./processV7M0Baseline');
async function inspectExcelEvidence(db) {
...
async function applyExcelEvidence(db) {
  const [[lock]] = await db.execute("SELECT GET_LOCK('mdm_excel_evidence_migration_v1',10) acquired");
  if (Number(lock.acquired) !== 1) throw failure('DEFINITION_ANALYSIS_MIGRATION_BUSY', 409);
  try {
    const before = await inspectExcelEvidence(db);
    ...
    const after = await inspectExcelEvidence(db);
    ...
    return inspectExcelEvidence(db);
  } finally { await db.execute("SELECT RELEASE_LOCK('mdm_excel_evidence_migration_v1')"); }
}
module.exports = { inspectExcelEvidence, applyExcelEvidence, MIGRATION_KEY, tables };
