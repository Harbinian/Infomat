...
async function applyAnalysisAi(db) {
  const [[lock]] = await db.execute("SELECT GET_LOCK('mdm_analysis_ai_migration_v1',10) acquired");
  if (Number(lock.acquired) !== 1) throw failure('DEFINITION_ANALYSIS_MIGRATION_BUSY', 409);
  try {
    const before = await inspectAnalysisAi(db);
    ...
    const after = await inspectAnalysisAi(db);
    ...
    return inspectAnalysisAi(db);
  } finally { await db.execute("SELECT RELEASE_LOCK('mdm_analysis_ai_migration_v1')"); }
}
module.exports = { inspectAnalysisAi, applyAnalysisAi, MIGRATION_KEY, tables };
