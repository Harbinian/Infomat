...
async function termGovernanceScopeAsync(alias, req, options = {}) {
  const global = options.global === undefined ? await hasGlobalViewAsync(req) : options.global;
...
async function findGovernableProcessAsync(req, processId, options = {}) {
  const scope = await termGovernanceScopeAsync('p', req, options);
  return db.prepare(`
...
async function validateGovernableProcessAsync(req, res, processId) {
  ...
  const process = await findGovernableProcessAsync(req, processId, { global: globalManager });
...
router.get('/processes', requireAuth, (req, res) => {
  return runAsyncAction(res, async () => {
    const scope = await termGovernanceScopeAsync('p', req, { global: await isAdminAsync(req) });
...
router.get('/', requireAuth, (req, res) => {
  return runAsyncAction(res, async () => {
    ...
    const scope = await termGovernanceScopeAsync('p', req);
    if (scope.sql) {
...
router.post('/', requireAuth, (req, res) => {
  return runAsyncAction(res, async () => {
    ...
    if (!await validateGovernableProcessAsync(req, res, normalizedProcessId)) return;
...
router.put('/:id', requireAuth, (req, res) => {
  return runAsyncAction(res, async () => {
    ...
    if (!await isAdminAsync(req) && (existing.created_by !== req.session.userId || existing.status !== 'pending')) {
