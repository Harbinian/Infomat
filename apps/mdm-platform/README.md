...
旧问题池仓储对已关联问题的关闭、重开、备注、问题点动作、术语动作和生成覆盖均拒绝，并在问题行锁内检查。旧列表不暴露新来源或已关联问题，旧详情/写接口统一返回404；新受限接口核对全部关联来源后才显示内容。未关联的旧问题维持原有行为；V1—V3及跨部门承接退役边界保持。新来源问题关闭的主体及审批依据尚待确认，因此没有关闭入口，也不以待办完成或本轮未检出自动关闭。
...
- POST `/api/analysis/issues/:issueId/review`：当前仅保留服务端拒绝门槛。新来源问题的有权主体及审批依据尚未确认，因此不执行关闭、复核通过/不通过或重开；接口不接受角色名称或客户端声明作为授权。
