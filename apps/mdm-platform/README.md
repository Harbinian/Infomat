...
已删除3000的V1至V3编制器、旧版导入和承接办理接口，以及SQLite时代的系统、能力、流程、地图视图、组织岗位、产品分类、属性、外部系统和集成路由。旧接口不再注册；共用的流程读取、提交、审核和发布接口只接受V7，旧记录返回`410 LEGACY_PROCESS_RETIRED`。历史数据库、标识、审核记录及共享迁移结构保留，不清空、不猜测转换。

当前主线为3001编制V7，用户主动下载上传，3000核对、审核、发布，再下载程序文件。3001自身的历史JSON导入升级能力不在本次3000退役范围。

本次修改工作区代码，不执行正式数据库变更。运行中的3000需要在完成验证后按既有运维入口切换，才能使后端退役生效。旧发布包、备份及私有配置保持原位；历史说明中相反的旧编制和SQLite可运行描述不再适用。
