...
- 文档结构化输出：在网页中按节点录入制度目的、范围、承继关系、术语、多个流程、多个业务行为、跨部门回写承接和附表结构；流程的 L1 能力域和 L2 业务能力从已有映射关系枚举选择，暂不开放新增；术语、流程和业务行为支持在草稿/退回修改状态下回填编辑、删除或作废，发布后投影到能力-流程-行为结构
- 跨部门待办：给其他部门派发待办
