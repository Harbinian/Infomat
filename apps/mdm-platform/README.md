...
- `MDM_IDENTITY_READ_MODEL=mysql` 目前切换登录、`/api/org/session`、`/api/org/me`、本人密码状态、本人改密、管理员用户/部门/权限读写接口、`/api/roles` 角色读写接口、通用 `requirePermission` 权限中间件，以及角色工作台的当前用户、角色、部门和权限读取。`auth.js` / `access.js` 已提供 MySQL-aware 异步权限和角色码 helper；业务路由仍需逐步从同步身份检查迁过去。
