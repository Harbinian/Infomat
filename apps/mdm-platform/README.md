...
- `MDM_IDENTITY_READ_MODEL=mysql` 目前切换登录、`/api/org/session`、`/api/org/me`、本人密码状态、本人改密、管理员用户/部门/权限读写接口、`/api/roles` 角色读接口，以及角色工作台的当前用户、角色、部门和权限读取；角色写接口尚未迁移，在该模式下会显式拒绝，避免 MySQL 读、SQLite 写混用。角色工作台流程待办数据层仍在后续迁移中。
