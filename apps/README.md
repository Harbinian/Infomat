...
4. 修改信息表收集服务时，进入 `apps/information-collection-service/`；身份数据只读复用现有 MySQL，信息收集权限和业务数据不得写入 MDM 治理表。
5. 组织资料以 `docs/organization/` 下的真源为准。
6. 流程映射以 `docs/norms/` 下的输入基线为准；应用只消费快照或同步结果。
