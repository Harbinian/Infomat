param(
  [string]$BaseUrl = 'http://127.0.0.1:33001',
  [switch]$Headed
)

$ErrorActionPreference = 'Stop'
$appRoot = Split-Path -Parent $PSScriptRoot
$scenarioPath = Join-Path $PSScriptRoot 'flow-pending-browser-scenario.js'
$artifactRoot = Join-Path $appRoot 'output/playwright'
$sessionName = "infomat-flow-pending-$PID"
...
try {
  ...
  throw "Cannot reach the candidate 3001 instance at $BaseUrl. This script does not start services. Error: $($_.Exception.Message)"
}
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "Flow pending-edit browser regression failed. Exit code: $LASTEXITCODE. Failure screenshot (when available): $artifactRoot\flow-pending-browser-failure.png"
  }
} finally {
...
Write-Host '3001 flow pending-edit browser regression passed.'
