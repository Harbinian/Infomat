...
for (const target of [behavior, form, field, {kind:'relation',ref:'flow_return'}, {kind:'data',ref:'data_fixture_application'}, {kind:'form-item',parentRef:'form_application',ref:'item_amount'}]) {
  const editorKind = {behavior:'behavior-properties',relation:'relation-properties',data:'data-object-properties'}[target.kind] || 'review-properties';
  const session = {candidateKey:'a',editorKind,entityRef:editorKind === 'review-properties' ? Review.targetKey(target) : target.ref};
  assert.equal(Review.isEditingTarget(session,target,'a'),true);
  assert.equal(Review.isEditingTarget(null,target,'a'),false,'Discarded sessions must not leave editable inputs');
  assert.equal(Review.isEditingTarget(session,target,'b'),false,'Another candidate cannot supply the edit session');
  assert.equal(Review.isEditingTarget({...session,entityRef:'other'},target,'a'),false,'Another object cannot supply the edit session');
}
