...
当前导出结构为`docs/contracts/process-governance-v5.schema.json`。格式以单个`process`为根对象，不使用`processes[]`，从结构上保证一份JSON只对应一个流程。v1至v4结构规则继续作为兼容导入规则。
...
4. 导出时统一生成`process-governance-v5`，导出结果可重新导入并适配回编辑工作台。v1至v4表单只补待确认状态和空关系，不根据名称、编号、明细数量或页面顺序推断业务关系。
...
`process-governance-v5`继续保留`actor_assignment_mode`、`actor_department_data_ref`和`actor_position_rule`，并移除活动结构中的`cross_department_handoffs`。数据行为关系统一保存在`data_objects[].behavior_links[]`，流程先后统一保存在`flow_relations[]`。旧v1至v4文件由3001在当前页面内存中转换，源文件不修改；本次不修改3000业务功能。

`INFORMATION_TYPES`为每个信息类型保留稳定的`value`、标准`label`和页面`example`。下拉选项使用“标准名称（例：具体例子）”，关系总览只读取标准`label`。页面例子不得写入`information_type`或其他导出字段，因此不改变Schema，也不需要迁移旧文件。
...
- 当前打开的文字分区和流程步骤子分区；流程步骤只包含业务流程、流程关系、输出物与数据、关系总览。
- 当前业务行为、流程关系、输出物与数据、术语及表单或记录的稳定引用；表单右侧同时渲染全部字段分组，不再保存“当前表结构标签页”状态。
- 当前侧边栏拖动状态只保存待移动条目、原位置、目标位置和输入方式；确认前不修改数组，取消后立即丢弃。
- 从`/api/enums`读取的花名册岗位目录及当前加载状态。
- 图例示例展开状态和预览展开状态。

候选列表只存在当前页面运行态。切换候选时保留本次运行内的编辑结果，刷新后全部清空。

页面外壳使用`app-shell`两列网格：`task-sidebar`固定300px，`main-column`使用`minmax(0, 1fr)`占满剩余宽度。`body`设置`min-width: 1920px`。左侧任务栏在视口内常驻并独立滚动，右侧编辑区保持`min-width: 0`，宽表格只在自身容器内滚动。
...
页面不设置低分辨率专门提示和对应媒体查询。低于1920×1080时由页面滚动承载桌面布局；页面不对低分辨率和移动端布局作保证，但不得阻断操作或丢失内容。
...
业务行为、流程关系、输出物与数据、术语分别通过`activeBehaviorRef`、`activeRelationRef`、`activeDataRef`和`activeTermRef`记录当前条目。四类页面统一渲染`record-workbench`：左侧`record-selector`显示条目，右侧只渲染当前条目的编辑卡。`sortableDragState`在鼠标、Pointer Events触屏或键盘开始时记录原位置和目标位置；移动到侧边栏边缘时调用`scrollBy()`。确认且位置变化后才用`splice()`移动完整对象并调用`touch()`；取消或位置未变化时不改数组、不调用`touch()`。任何顺序调整都不得重建技术标识、改写对象内容或更新跨数组引用。流程图仍按明确关系计算，不得把数组顺序解释为流程先后。
...
- `public/process-diagram.js`把当前`process-governance-v5`内存对象转换为图形元素。转换函数接受部门目录顺序，但不得修改输入对象。
...
- 布局使用Cytoscape`preset`模式。前端先计算普通流程关系层级、泳道高度和节点坐标，再一次性装载图形。显式回路和内部调用返回不参与主线层级计算。
...
- `ProcessDiagram.buildGraphModel()`只为当前页面增加`rawLabel`、显示用换行、节点和标签尺寸、线路轨道、闭环核对项、布局碰撞结果和视野建议。连续中文按字符换行，并同时设置Cytoscape的`text-overflow-wrap: anywhere`；显示换行不得写回业务字段。泳道标题、流程标题、业务节点、外部门节点、会签标记和关系文字分别使用14px、16px、15px、14px、12px和13px。普通节点按行数增加高度，判断和并行菱形按内接文字区域同步扩大宽高。
- 图形节点只读取`behavior_name`作为主标题，不读取`behavior_description`。补充说明只在文字编制中显示，避免增加主图信息密度。
- 关系标签最大宽度为220图形单位。相邻层级中心间距取“前后节点半宽之和＋220＋48”和440中的较大值。所有关系禁止使用`unbundled-bezier`。`allocateRelationRoutes()`按每个层级内节点的稳定显示顺序计算相对上下位置：判断结果与起点同高时可以使用直线，目标位于上方或下方时分别分配`route-upper`或`route-lower`；跨层并行路线根据起点所在一侧继续向外分配轨道。需要绕行的判断分支增加`route-forward-branch`，跨层或同端点重复的并行关系增加`parallel-orthogonal-edge`；两类关系分别使用`upward`或`downward`无圆角`taxi`折线，`taxiTurn`读取当前轨道预留距离。其他`route-upper`和`route-lower`关系使用`segments`及两个显示用折点；显式回路和部门内调用返回以直线段从节点区下方绕行。同一方向的下一条轨道在上一条标签高度之外再增加24图形单位，泳道高度同步预留上方和下方轨道空间。并行内含判断时，内层判断路线先占用内侧轨道，跨层并行绕行线使用更外侧轨道，不得与判断路线或关系文字相交。
- 非回路闭环中的关系进入`reviewItems`，提示关系类型核对并保留`focusKind: relation`和原`focusRef`。该项只影响页面提示和定位，不进入结构校验、不阻止导出、不自动改为回路，也不修改关系箭头的类型样式。
- 业务行为按固定执行部门进入对应泳道。普通流程关系两端部门不同时，边使用`cross-lane-relation`样式；图中不生成待办节点，也不读取`migration.legacy_cross_department_records[]`。
...
- Cytoscape节点设置为不可抓取。图形只允许平移、缩放、适应画布、重置视图和展开查看。业务行为和普通流程关系可以定位到对应文字卡片；内部流程调用不跳转到文字编辑区。初始化和重置时先完整拟合画布：拟合缩放比例不低于0.60时保留全图，否则改为拟合泳道表头中心锚点和前两层节点并显示聚焦提示；“适应画布”不使用该阈值，始终拟合全部元素。只读预览允许缩小至0.03，以保证约40个行为的宽流程能够完整进入画布。
...
| 跨部门先后和返回 | `flow_relations[]`中的普通流程关系 |
| 跨部门数据传递和返回 | `data_objects[].behavior_links[]` |
| 隐式保留并仅供流程图预览的同部门调用 | `internal_process_calls[]` |
| 表单或记录、主表／明细表、填写项 | `forms[].areas[].items[]` |

## 7. 跨部门行为、旧记录转换和部门内调用预览

- `flowItems()`只返回`behaviors[]`。`actorAssignmentMode()`为固定部门，且`current_actor_role`解析出的部门与`process.owning_department`不同时，页面为该行为增加跨部门标签；不创建独立字段或第二个流程对象。
- `flow_relations[]`是流程先后的唯一事实来源。跨泳道、返回、条件和并行仍使用同一关系集合；`data_objects[].behavior_links[]`是数据创建、更新和使用的唯一事实来源。
- `behaviorOptions()`遍历全部`behaviors[]`，生成“序号＋执行部门＋行为名称”标签；外部门行为增加“跨部门”标记。数据关系的创建、更新和使用下拉均复用该函数，不按泳道过滤。
- `normalizeV1()`接受v1至v5。v1至v4的`cross_department_handoffs[]`先通过稳定引用规范化，再由`convertLegacyCrossDepartmentHandoffs()`转换。前置输入生成“外部门行为→锚点行为”，后续承接生成“锚点行为→外部门行为”，需要返回时生成“外部门行为→恢复行为”。
- 旧记录未关联外部门行为，但部门和办理动作明确时，转换器使用由`handoff_ref`派生的稳定`behavior_ref`创建业务行为。锚点有效时，前置输入行为插入锚点之前，后续承接行为插入锚点之后；同一锚点的多条同方向记录按旧`cross_department_handoffs[]`顺序插入。该顺序只恢复v4列表的原显示位置，不替代或改写`flow_relations[]`。
- 缺少方向、锚点、恢复位置或数据关系时，转换器不猜测，记录`conversion_status=needs_manual_completion`和逐项原因。锚点不存在时，转换器可以保留已明确的外部门行为，但不得把其数组位置解释为流程先后关系。
- `ensureMigratedSequenceRelation()`和`ensureMigratedDataLink()`先按端点、操作和稳定引用查重。`migration.legacy_cross_department_records[]`只读保存源记录、状态、原因和结果引用；v5重导入只规范化已有迁移记录，不再次执行转换。
- 转换只操作导入对象的规范化副本；源文件不写回。迁移记录不传给评分图、流程图或当前流程关系计算。
...
- `externalActorDepartment()`只对固定部门模式的`current_actor_role`与归口部门进行比较，结果仅用于标签、评分和跨泳道样式。改回归口部门、全公司通用或动态责任部门时，不删除普通流程关系或数据关系。
- 归口部门选择器不通过通用`input`绑定直接写入草稿。`owningDepartmentChangeImpact()`先统计固定和动态执行主体、能力归类、正式工作角色及历史部门内调用，并向用户显示旧值、新值、重置范围、保留范围和后续处理。用户确认后，`buildOwningDepartmentResetDocument()`在深拷贝中清空固定执行部门和岗位，把动态责任部门恢复为未选择的固定模式，并把能力归类恢复为未归类；`company_wide`行为、普通流程关系和数据关系保持不变。前端只有在副本通过`/api/validate`后才替换当前草稿；取消、版本不一致、服务异常或校验失败均不得修改原草稿。
...
- `dataFlowConsistencyDetails().isAvailableBeforeBehavior()`校验动态来源数据：数据必须存在有效产生行为或`source_relations[]`中的可用起点，并且沿非回路普通流程关系到达当前行为；当前行为自身、后续行为和并行兄弟路线产生的数据均不通过。
...
- `renderForms()`为每张表单渲染一个`form-field-table`，按`forms[].areas[]`顺序使用多个`tbody`分组显示主表字段、全部明细表字段和归属待确认字段。列固定为序号、字段名称、数据类型、字段归属、必填、填写说明、排序和删除；明细表标题及排序操作位于分组标题行。字段名称单元格提供“数据关系”入口，`renderFieldRelationEditor()`统一维护`business_data_ref`、`value_origin_mode`和`source_links[]`；这三项不再占用表格列。`source_links[]`使用`source_type=process_data|external_system`区分本流程数据和外部系统：前者引用`source_data_ref`，后者把该引用置空并记录`source_system_name`和`source_data_name`，不生成本流程行为或数据行为关系。旧v5链接没有`source_type`时规范化为`process_data`。主表`area_title`仅作为历史兼容值保留。
...
- v1至v3表单导入时把缺失状态写为`unspecified`；v4和v5合法状态原样保留。源文件只读，升级结果只存在当前页面内存。
...
- 没有有效前序来源的行为显示`trigger`编辑控件，标签为“流程如何开始”。存在有效非回路普通流程关系时，页面通过`behaviorEntrySummary()`生成只读进入说明；历史非空`trigger`保留在只读历史补充中。
...
- `dataFlowConsistencyDetails()`是数据先后规则的唯一实现，供业务行为只读摘要、数据行为选择器、`businessWarnings()`和`evaluateContent()`共同使用。它按有效`sequence`、`condition`和`parallel`普通流程关系构建有向图，排除`loop`，不读取迁移记录。
- 普通数据的使用行为必须位于唯一有效产生行为之后。产生行为等于使用行为时判为自身引用；使用行为可到达产生行为时判为前序引用后续；两者互不可达时判为并行或无序引用；双向可达时判为非回路闭环，以上情况均不允许新增。
- 无本流程创建行为的数据通过`source_relations[]`说明流程开始时可用、从某个行为起可用或待确认。可用位置之前以及与可用位置无明确先后关系的行为不能引用。
- 创建、更新和使用行为选择器列出全部实际业务行为，包括跨部门行为；标签显示序号、执行部门和行为名称。导入历史不合规值时，当前值继续显示并标记待整改，页面不得在导入时删除。
- 数据关系只写入`data_objects[].behavior_links[]`。行为侧历史输入输出引用仅用于v1至v3转换，v5不得恢复双写同步函数。

## 9. 校验分层

3001导出前执行：

- 新建流程通过`/api/template?version=process-governance-v5`读取服务端空白模板，避免前后端分别维护结构。
- 新建和导出前调用`/api/schema`确认服务端默认结构版本为`process-governance-v5`；版本不一致时停止后续请求并保留当前页面内容。接口返回HTML或其他非JSON内容时，前端转换为可操作的服务异常提示，不直接暴露JSON解析错误。
- JSON Schema校验。
- 技术标识格式校验。
- 单文件内行为、数据对象、表单引用检查。
- 一份文件一个流程检查。
...
- 数据产生行为与使用行为的流程先后关系。
- 旧跨部门记录是否存在无法唯一转换的方向、锚点、恢复位置或数据关系。
- 跨部门行为是否缺少执行部门、岗位或有效普通流程关系。
- 执行岗位是否为空或仍为当前花名册未收录的历史值。
- 表单设计状态是否待确认、字段归属是否待确认，以及多张明细表中哪一张缺少区分标题。

`businessWarnings()`返回`message`、`suggestions[]`和定位元数据。`message`只保存系统能够确认的当前情况，`suggestions[]`至少包含1条可执行建议；多条建议按页面顺序显示，不合并为含糊句子。基础字段使用`editorSection + focusPath`定位；业务行为、流程关系、数据对象、表单和表结构使用`focusKind + focusRef`选择对象，再用`focusPath`定位具体字段。旧迁移记录无法转换时，分别定位业务行为、流程关系或数据关系补录位置。流程入口说明缺失时定位`trigger`；数据时序问题定位数据对象的创建、更新或使用行为。执行部门和岗位选择器通过`data-focus-key`映射到`current_actor_role`，复选项通过`data-list-bind`映射到数组字段。导出确认框使用同一组当前情况和建议，不另写一套文案。

`parallelStructureDetails()`是并行完整性的唯一计数入口，供结构评分和`businessWarnings()`共同使用。并行开始按不同`to_behavior_ref`统计从该节点发出的有效`parallel`路线；并行汇合按不同`from_behavior_ref`统计流入的有效`parallel`普通流程关系。计数结果同时保留与该控制节点相连、但`relation_type=sequence`的关系。`parallelSplitGuidance()`和`parallelJoinGuidance()`根据同一结果生成当前情况、建议和全部关系类型`focusPaths`：存在可改类型的顺序关系时建议修改现有关系，转换后仍不足时才补充新增建议。

`dataFlowConsistencyDetails()`是数据时序的唯一判断入口。跨部门前后和返回都已转换为普通流程关系，因此与本部门行为使用同一可达性规则。结构评分、数据选择器和导出检查复用同一问题列表；历史不合规引用在当前页面内存中保持原值，只有用户主动修改对应数据关系时才归一该关系。
...
判断节点出口检查只统计从该节点发出的`sequence`、`condition`和`loop`普通流程关系。出口数量不足时报告当前数量；同一判断节点存在多条无条件顺序关系时，从第二条开始逐条定位。每条流程关系分别检查关系类型、起点行为和目标行为；`condition`单独检查判断条件，`loop`单独检查回路触发条件，`sequence`允许空`condition`作为默认继续路径。检查不要求目标节点成为判断节点。计算主线入口和出口时排除显式`loop`，避免退回关系把正常主线误判为没有入口或出口。

MDM平台负责跨对象和最终可执行性校验。真实责任人负责审核业务事实。

JSON Schema结构错误和单文件技术引用错误分别提示。结构错误提示维护人员检查导出格式；只有技术标识重复或本文件引用断开时，页面才提示“技术引用已断开或重复”。

顺序调整、提示定位和判断出口识别本身不改写对象内容。v1至v4只执行本规格明确的兼容字段映射；兼容验证必须确认导入后可点击定位，重新排序只改变`behaviors[]`、`flow_relations[]`、`data_objects[]`或`terms[]`中对应对象的数组位置，所有稳定引用和业务字段保持不变。

## 9.1 流程评审五方面标准实现

`public/structure-score.js`以`process-review-readiness-v5`固定维护五方面名称、说明、现场确认口径和统一通过口径。`evaluateReviewReadiness()`接收当前流程只读对象、普通业务提示、业务行为可执行性提示、评审专用路由提示和现有技术结构结果，按定位信息把问题归入边界、行为责任、逐动作可执行性、条件与跨部门流转、数据与表单五方面。函数不得修改输入对象。

`renderReviewReadinessPanel()`在“流程评审”顶部渲染总操作状态和五张按顺序展开的标准卡。结构错误显示为阻断；业务缺项显示为可下载提示；没有自动缺项时显示“待部门确认”。技术检查进行中或服务不可用属于运行状态，不转换为用户业务问题。问题按钮继续使用`focusExportWarning()`返回原字段。

`behaviorExecutabilityDetails()`只读检查`node_type=action`的业务行为。空白或占位的`behavior_description`记录为说明缺失；命中“确保……可以完成”“保证……真实性、合理性等定性结果”的内容记录为抽象定性表述；其他内容未出现受控的具体业务动作词时记录为没有具体动作。`reviewBehaviorExecutabilityWarnings()`把每个问题转换为第三方面的独立提示，并将`focusPath`指向对应`behaviors.<index>.behavior_description`。自动词法检查只发现明显问题；处理对象、结果和每个动作是否完整仍由业务部门确认。

流程入口通过非回路关系和跨部门关系计算。末端数量可以大于1；每个末端业务行为分别使用`completion_standard`确认结束结果。只有没有可识别末端时才提示核对，不得自动补线、删除分支或把普通关系改写成回路。

`loopExitDetails()`只读构建排除显式`loop`的前进关系图。对每条回路，它从返回位置计算可到达节点，并从回路检查节点反向计算可达节点；两者交集构成本层循环体。回路检查节点必须至少有一条指向循环体外的前进关系。一个循环体严格包含另一个循环体时，内层循环标记为嵌套；内层退出可以仍位于外层循环体内，外层退出必须位于外层循环体外。

`parallelRouteSafetyDetails()`从每个`parallel_split`的不同并行目标出发，计算全部路线能够共同到达的`parallel_join`，并按最大距离、总距离和节点顺序选择最近共同汇合。遍历每条路线时在共同汇合处停止；共同汇合前没有后续前进关系的业务行为或判断节点记录为提前结束位置。内层并行先选择内层最近共同汇合，因此内层汇合结果可以继续进入外层共同汇合。

`reviewRoutingWarnings()`把缺少本层循环退出、并行路线没有共同汇合和并行路线提前结束转换为第四方面的评审专用提示。提示只由`renderExportCheck()`在“流程评审”调用，不加入`businessWarnings()`，因此普通导出确认框和其他编制标签保持原行为。

`public/review-pattern-diagrams.js`以纯数据定义节点组合表对应的九类图例，并把每类图例转换为仅用于展示的内存流程对象。`renderNodeCombinationGuide()`只在“流程评审”渲染九个画布；`mountReviewPatternDiagrams()`复用`ProcessDiagram.mount()`绘制与跨职能流程图一致的泳道、业务行为、判断、并行、关系线和会签标记。每张图支持平移、缩放、适应画布、重置视图和展开查看。离开“流程评审”或页面重新渲染前，`destroyReviewPatternDiagrams()`销毁已建立的图形实例。

五方面检查结果只使用现有`structureScoreState`生命周期和当前渲染数据，不建立新的持久状态。图例的布局坐标、缩放比例和展开状态只存在当前页面内存。重新检查、展开卡片和点击定位不得调用`touch()`，不得修改`exported_at`、JSON、技术引用、当前选择或未导出状态。结构化学习评分折叠为辅助区域，不参与`operationStatus`计算。
...
3. 页面调用`/api/schema`确认前后端均使用`process-governance-v5`，再分别把当前副本和规范化回读副本提交到`/api/validate`。
...
判断出口评分使用现行3001规则：一条无条件`sequence`可以作为默认继续路径，`condition`和`loop`必须填写条件。出口只来自普通流程关系；评分只检查结构是否完整，不要求目标节点为`decision`，也不判断条件是否互斥或业务做法是否正确。
...
跨部门行为完整性子项满分5分。没有跨部门行为时按不适用获得5分；存在跨部门行为时，每个行为分别检查固定执行部门、有效岗位和至少一条有效普通流程关系。评分函数对输入对象只读，测试必须在评分前后比较规范化JSON。

评分模块返回问题的维度、当前情况、`suggestions[]`、影响和定位元数据。页面使用现有`focusExportWarning()`定位基础字段、业务行为、流程关系、数据对象、表单或表结构；无法定位具体输入控件时退回对象卡片或分区。重复的评分扣项和普通导出提示按文案及定位元数据去重。

服务端继续编译v1至v5规则；当前默认模板和导出使用v5，v1至v4仅用于兼容导入。旧交接字段先规范化为统一历史记录，再按方向转换为普通流程关系和数据行为关系；缺少唯一锚点、恢复位置或数据去向时保留原值并产生补录提示，不猜测正确答案。v1、v2表单在页面内存中补`form_design_state=unspecified`。
