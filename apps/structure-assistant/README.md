...
本应用不属于3001。3001的启动、新建、导入、校验和导出不依赖本应用或端口3004；本应用只能作为3001公开`process-governance-v5`结构规则的一个独立客户端。助手内部结构客户端显式请求3001的v5健康检查、Schema和空白模板接口，不依赖3001当前默认的v7版本；端口3004的`/structured-tool/`认证代理仍打开3001当前v7页面，不把该页面降级为v5。助手可以询问并保留v5的普通流程关系、数据行为关系、来源线索、表单操作和字段数据关系；字段取值来源可以是本流程数据，也可以是明确的外部系统和来源数据名称。助手不得自行推断这些业务事实，也不得为外部系统来源虚构本流程行为，不得创建或修改`migration`中的旧跨部门记录。

## 用户教程

五账号试点人员按[《MDM-AI助手流程与数据治理使用教程》](../../docs/training/2026-08-17-MDM-AI助手流程与数据治理使用教程.md)完成Key绑定、对话填报、独立结构预审、3001核对和案例验收。教程同时说明助手当前v5成果、3001当前v7成果与3000现行v3承接能力之间的停止边界。
