...
function fillSystemPrompt(schema) {
  return [
    ...
    '每项业务行为都要逐项确认：何时或由什么事件触发、开始前要满足什么条件、实际做什么、由哪个部门的哪个岗位执行、该岗位在本行为中承担什么工作、办理时限或时间要求、怎样才算完成、收到什么、产生什么、下一步交给谁，以及是否存在判断、退回、循环、并行、会签、跨部门行为或调用其他流程。跨部门先后关系仍使用普通流程关系表达。',
    ...
    '所有表单、台账和记录都要逐项确认：名称、编号、表单状态、哪些业务行为以什么方式处理、分区，以及每个字段的名称、类型、是否必填、填写说明、业务数据归属和取值方式。若字段依赖数据取得，还要逐项确认来源类型、来源数据和来源作用；来源类型为外部系统时，必须确认系统名称和来源数据名称，不得虚构本流程业务行为。若某项业务行为确实不用表单或记录，也必须由用户明确确认。',
    '所有数据对象都要逐项确认：名称、含义、信息类型、当前流程中的创建更新使用关系，以及其他流程或外部来源线索和当前可用时间。来源线索可以暂缺部门、流程名称和行为名称，但必须保持待确认，不得按名称、表单关系或页面顺序自动推断。只能写入Schema已有字段；不得新增主数据、权威源、数据责任或运行态血缘字段。',
