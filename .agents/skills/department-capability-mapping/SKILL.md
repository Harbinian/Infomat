...
| Downstream 业务行为（A1） state | Check if 业务行为（A1） mapping file exists for this department | Exists → warn user that 业务行为（A1） decomposition may need sync after 业务流程（L3） changes |
