...
| 业务行为（A1） | text | Role completing one independently describable task action; when `输入来源部门` or `输出目标部门` is non-empty, append `【跨部门：入←{输入来源部门} 出→{输出目标部门}】` (omit empty side) to the visible text for review |
...
### Cross-department Marker Rule

When `输入来源部门` or `输出目标部门` is non-empty, the visible `业务行为（A1）` text shown to reviewers (tables, node labels) must include a cross-department marker. Do not rely on reviewers to cross-check separate columns.
...
- When `输入来源部门` or `输出目标部门` is non-empty, the visible 业务行为（A1） node label must include a cross-department marker (short `【跨部门】` is acceptable). Tooltip and the detail table must show full `输入来源部门` / `输出目标部门` values.
