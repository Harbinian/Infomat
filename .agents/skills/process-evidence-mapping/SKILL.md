...
引用校验覆盖各类对象、证据、表单父子关系及流程归属。旧草稿若含不存在或类型错误的引用，必须明确报错并回源修正；不自动改写历史文件或猜测对应对象。
...
问题的对象类型和标识必须指向草稿中实际存在的记录。尚未创建的表单或跨部门承接关系，其问题先关联已有业务行为；审批详情问题关联实际的 `detail_ref`。`target_block` 和 `target_field` 保留后续需补充的目标，不为满足引用校验而补造正式对象。
...
未解决状态以 v2 为准，不因映射文本中出现同名内容而关闭问题，也不从派生 Markdown 回读处理状态。只有 `user_decision=不是问题` 且填写 `user_reason` 时才从未解决视图移除；计划修改或专项确认仍保留。旧数组输入继续支持，但同名内容也不能作为关闭依据。

**不得做**：不得把 Markdown 当机器合同、流程输入基线或长期真源；不得默认写入
`docs/norms/流程治理/`。

**下一步条件**：人工阅读视图与 v2 中按上述规则计算的未解决问题数量一致后进入人工确认边界。
