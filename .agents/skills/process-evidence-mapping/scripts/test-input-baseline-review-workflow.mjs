#!/usr/bin/env node
/**
 * Regression checks for process input baseline review extraction and todo markdown.
 *
 * Usage: node .agents/skills/process-evidence-mapping/scripts/test-input-baseline-review-workflow.mjs
 */
...
const runDir = join(root, 'artifacts', 'process-input-baseline-review', 'test-gltx-cw-01');
const todoPath = join(runDir, '输入基线问题待办.md');
...
const genericRunDir = join(root, 'artifacts', 'process-input-baseline-review', 'test-engineering-generic');
...
function countTodoRows(markdown) {
  return markdown
    .split(/\r?\n/)
    .filter((line) => line.startsWith('| IBR-')).length;
}
...
const staleMappingFixture = [
  '# 财务部部门-能力-流程-系统映射关系（待确认测试夹具）',
  '',
  '## 业务能力—流程—系统映射表',
  '',
  '| 序号 | 部门（D1） | 能力域（L1） | 业务能力（L2） | 业务流程（L3） | 制度依据（文件号/条款） | 应用系统（S1） | 系统设计依据 |',
  '|---|---|---|---|---|---|---|---|',
  '| 4 | 财务部 | 成本管理 | 成本核算管理 | 月度产品成本核算与成本结转 | GLTX-CW-01-A《财务成本核算管理程序》§5.3-§5.4 | ERP | 测试夹具仅保留L3，不覆盖待确认A1 |',
  '',
  '## 业务行为（A1）映射（BBM增补）',
  '',
  '本夹具故意不包含 GLTX-CW-01 待确认 A1、归档和审批链内容，用于证明待确认待办会生成。',
  '',
].join('\n');
writeFileSync(mappingPath, staleMappingFixture, 'utf8');

runNode([
  '.agents/skills/process-evidence-mapping/scripts/run-process-input-baseline-review-workflow.mjs',
  '--input', sourceDoc,
  '--department', '财务部',
  '--mapping', mappingPath,
  '--out', runDir,
...
assert.equal(existsSync(todoPath), true, 'input baseline review todo markdown should be written');

const documentReviewItem = readJson(join(runDir, 'document_review_items.json'));
const roleReviewItems = readJson(join(runDir, 'role_review_items.json'));
...
assert.equal(documentReviewItem.department, '财务部');
assert.equal(documentReviewItem.source_file.endsWith('GLTX-CW-01-A财务成本核算管理程序.docx'), true);
...
assert.equal(chunks[0]?.allowed_downstream_use, 'review_only', 'review items must remain review-only even for GLTX files');
assert.equal(
  mappingItems.every((item) => item.source_boundary_flag === 'changxing_owned'),
  true,
  'input baseline review todo items should inherit source boundary from their evidence',
);
assert.ok(documentReviewItem.capability_review_items.some((item) => item.name.includes('成本核算')), 'should identify cost accounting reviewItem capability');
assert.ok(documentReviewItem.process_review_items.some((item) => item.name.includes('月度产品成本核算')), 'should identify monthly product cost process input baseline review');
assert.ok(documentReviewItem.behavior_review_items.some((item) => item.name.includes('处理盘盈盘亏')), 'should identify inventory gain/loss behavior reviewItem');
assert.ok(documentReviewItem.behavior_review_items.some((item) => item.name.includes('处理废品损失')), 'should identify scrap loss behavior reviewItem');
assert.ok(documentReviewItem.archive_review_items.some((item) => item.content.includes('保存年限30年')), 'should identify 30-year archive reviewItem');

const roleNames = roleReviewItems.roles.map((role) => role.name);
for (const expected of ['财务部', '财务部成本会计', '车间工人', '车间主任', '定额员', '经营发展部长', '行政人事部']) {
  assert.ok(roleNames.includes(expected), `role reviewItem should include ${expected}`);
}
assert.ok(roleReviewItems.roles.some((role) => role.name === '经营发展部长' && role.role_types.includes('批准角色')), 'should classify operation manager as approval role');
assert.ok(roleReviewItems.roles.some((role) => role.name === '行政人事部' && role.role_types.includes('数据提供角色')), 'should classify HR as data provider');
...
assert.match(diffReport, /相似度仅用于待确认排序，不是证据强度/);

for (const forbidden of ['"审批类型"', '"输入来源部门"', '"输出目标部门"']) {
  assert.equal(JSON.stringify(documentReviewItem).includes(forbidden), false, `review item JSON must not emit formal field ${forbidden}`);
}

for (const requiredHeader of ['编号', '部门', '来源文件/条款', '问题类型', '问题内容', '当前映射位置', '建议动作', '处理状态', '负责人/确认对象']) {
  assert.ok(todoMarkdown.includes(requiredHeader), `todo markdown should include ${requiredHeader}`);
}
for (const type of ['待确认A1', '审批链待确认', '受控传递待确认', '归档要求待补']) {
  assert.ok(todoMarkdown.includes(type), `todo markdown should include issue type ${type}`);
}
assert.ok(todoMarkdown.includes('该文件只保留未解决待确认问题'), 'todo markdown should state unresolved-only policy');

const firstRowCount = countTodoRows(todoMarkdown);
assert.ok(firstRowCount > 0, 'todo markdown should contain review item rows');
runNode([
  '.agents/skills/process-evidence-mapping/scripts/update-input-baseline-review-todo-md.mjs',
  '--review-items', join(runDir, 'mapping_diff_items.json'),
  '--mapping', mappingPath,
  '--todo', todoPath,
]);
...
assert.equal(coveredTodo.includes('处理盘盈盘亏'), false, 'formally covered reviewItem should not reappear in todo');
assert.equal(coveredTodo.includes('相关报表由财务部负责存档，保存年限30年'), false, 'covered archive reviewItem should not reappear in todo');
...
const engineeringDocumentPath = join(genericRunDir, 'document_review_items.json');
const engineeringRolesPath = join(genericRunDir, 'role_review_items.json');
const engineeringObjectsPath = join(genericRunDir, 'object_chains.json');
const pipeReviewItemsPath = join(genericRunDir, 'pipe-review-items.json');
...
assert.ok(engineeringDocument.process_review_items.some((item) => item.name.includes('产品设计需求定义管理')), 'generic process extraction should use engineering document title');
assert.equal(JSON.stringify(engineeringDocument).includes('处理盘盈盘亏'), false, 'generic process extraction must not emit finance-only reviewItems');
...
writeFileSync(pipeReviewItemsPath, JSON.stringify([{
  id: 'IBR-PIPEESCAPE',
...
for (let index = 0; index < 2; index += 1) {
  runNode([
    '.agents/skills/process-evidence-mapping/scripts/update-input-baseline-review-todo-md.mjs',
    '--review-items', pipeReviewItemsPath,
    '--mapping', mappingPath,
    '--todo', pipeTodoPath,
  ]);
}
const pipeTodo = readFileSync(pipeTodoPath, 'utf8');
const pipeLine = pipeTodo.split(/\r?\n/).find((line) => line.startsWith('| IBR-PIPEESCAPE'));
assert.ok(pipeLine.includes('表格内容 A \\| 表格内容 B'), 'todo markdown should escape pipe characters in reviewItem content');
