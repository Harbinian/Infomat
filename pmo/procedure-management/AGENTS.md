...
- 驾驶舱支持全公司视图和单域视图，顶部切换；管理层只读使用，不新增 CSV 导出。
- “关键发现”可以根据当前数据自动生成，但只能陈述流程数、承载关系和待补项等事实，不评价具体应用系统“最忙”“承载最多”或“最重要”。
- 页面延续米色暖宣纸视觉，使用赭红、鼠尾草、雾蓝和暗金作为主辅色；修改样式时继续使用现有 CSS 变量。
