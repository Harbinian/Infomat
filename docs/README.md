...
| 流程数据 | `docs/norms/{部门}部门-能力-流程-系统映射关系.md` | PMO 驾驶舱和 MDM 流程治理快照都不反向作为原始来源。 |
...
| 路径 | 职责 | 修改规则 |
|---|---|---|
| `docs/norms/` | 制度、流程、部门映射真源 | 改流程映射后运行 `node scripts/parse-sankey-data.mjs`。 |
...
| `docs/contracts/` | 当前主线契约文件 | 用于校验主线关系，不替代流程映射真源。 |
| `docs/integration/` | 集成、主数据和系统协同方案 | 可沉淀方案，不作为流程数据原始来源。 |
