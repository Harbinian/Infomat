# MDM MySQL 与待确认映射复核治理更新计划

日期：2026-06-16

## 背景

本轮工作服务于 MDM 平台中的流程治理能力。当前仓库仍处在“流程地图与数据地图的梳理与沉淀”阶段，正式流程输入基线仍是 `docs/norms/{部门}部门-能力-流程-系统映射关系.md`。待确认映射、待确认预览和复核记录只能作为治理过程资产，不得默认写入正式流程输入基线或正式展示页。
...
- `scripts/build-input-baseline-review-sankey-preview.mjs` 当前默认输出到 `docs/norms/{部门}部门能力流程系统桑基图.html`，会把待确认预览状态写入正式部门桑基图页面。
- `scripts/mark-sankey-preview-status.mjs` 当前批量扫描并修改 `docs/norms/*部门能力流程系统桑基图.html`，容易把临时待确认标记混入正式展示资产。
- 处置口径：待确认预览默认输出必须迁移到 `artifacts/process-input-baseline-review/<run-id>/preview.html`；正式 `docs/norms` 仅允许受控回源流程更新。

### P1：输入基线问题复核交互不符合治理口径

- 当前输入基线问题复核页仍存在“点选标签生成修正意见”和 `data-correction-fragment` 的拼接式长文本逻辑。
- 当前待确认清单偏平铺，缺少“部门 → 文档名称 → 问题类型”的治理分组。
...
- 原文摘要高亮主要来自待确认内容拆词，未覆盖角色、动作词、对象/交付物词。
- 处置口径：只有明确 `page` 字段才能显示页次；`Pxx` 只显示为内部抽取锚点，不能显示为原文段落号或块号。高亮来源扩展为待确认内容、角色词、动作词、对象/交付物词。
...
2. Failing Tests First
   ...
   - 增加输入基线问题复核测试：页面不得包含 `data-correction-fragment` 或“点选标签生成修正意见”。
   - 增加证据测试：`P71` 显示为内部抽取锚点，不显示“第71页”、原文段落号或块号。
   - 增加角色规则测试：裸普通角色进入 `原文定义不足`，三类领导角色例外。
   - 注册缺失脚本：`test:sankey-preview-status`。

3. Input Baseline Review Refactor
   - 把输入基线问题复核核心逻辑拆为纯规则函数：分组、锚点格式化、高亮词提取、角色定义状态判定。
   - 页面保存结构化字段，不再保存拼接式修正意见。
   - MySQL 表保留待确认运行、待确认项、摘录、决策，补充 `document_name`、`issue_type`、`definition_status`、`normalized_note`。

4. MDM Integration
   - 在 `apps/mdm-platform` 的流程治理 API 下增加输入基线问题复核接口。
   - 前端流程治理页新增输入基线问题复核区块，复用现有流程治理筛选和权限口径。
   - 待确认 JSON 仍留在 `artifacts/`；已确认流程映射仍只由受控回源流程更新。
...
- 不批量清理 `docs/norms` 现有改动；如需清理待确认预览标记，只做定向、可解释的最小处理。
- 不把待确认 JSON、待确认预览、输入基线问题复核结果视为正式流程输入基线。
- MySQL 按 8.0+ 处理。
