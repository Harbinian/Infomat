...
**Goal:** 将 mdm-platform 前端从灰度单色系改为 logo 驱动的红黑品牌配色（混合模式：深色头部 + 浅内容区）。

**Architecture:** 单文件 CSS 变量替换。仅修改 `mdm-platform/public/index.html` 中的 `:root` 变量、个别硬编码颜色值、topbar 结构（加 logo），以及复制 `logo.png` 到 `public/` 静态目录。
...
**Files:**
- Copy: `mdm-platform/logo.png` → `mdm-platform/public/logo.png`

- [ ] **Step 1: 复制文件**

```bash
cp mdm-platform/logo.png mdm-platform/public/logo.png
```

- [ ] **Step 2: 验证文件存在**

```bash
ls -la mdm-platform/public/logo.png
```

- [ ] **Step 3: Commit**

```bash
git add mdm-platform/public/logo.png
...
**Files:**
- Modify: `mdm-platform/public/index.html:9-21`
...
**Files:**
- Modify: `mdm-platform/public/index.html:36-49` (topbar CSS)
- Modify: `mdm-platform/public/index.html:192` (topbar HTML)
...
**Files:**
- Modify: `mdm-platform/public/index.html:78-90` (tabs CSS)
...
**Files:**
- Modify: `mdm-platform/public/index.html:152` (table hover)
- Modify: `mdm-platform/public/index.html:162` (tag.red)
...
**Files:**
- Modify: `mdm-platform/public/index.html:606` (柱状图 color)
- Modify: `mdm-platform/public/index.html:609` (饼图 color 数组)
...
```bash
cd mdm-platform && node server/index.js
```
