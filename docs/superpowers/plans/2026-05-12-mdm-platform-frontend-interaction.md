# MDM 平台 Frontend Interaction Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild the MDM 平台 frontend from a flat 7-tab SPA into a role-differentiated, multi-layer information architecture with L1→L2→L3 page navigation, field-level rejection, and full conflict coordination workflow. Backend changes support new tables/endpoints for rejection reasons, conflict assignments, coordination history, and role-based data filtering.
...
**Files:**
- Modify: `mdm-platform/server/db.js:229-241`
...
**Files:**
- Modify: `mdm-platform/server/db.js` (at end of exec block, before `);`)
...
**Files:**
- Modify: `mdm-platform/server/db.js`
...
**Files:**
- Modify: `mdm-platform/server/routes/todos.js`
...
```bash
cd mdm-platform && npm run smoke
```

- [ ] **Step 3: Commit**

```bash
git add mdm-platform/server/routes/todos.js
...
**Files:**
- Modify: `mdm-platform/server/routes/mappings.js` (append before `module.exports`)
...
**Files:**
- Modify: `mdm-platform/server/routes/conflicts.js` (full rewrite)
...
In `mdm-platform/server/index.js`, after `registerRouteIfExists('/api/conflicts', 'conflicts');`, add:
...
```bash
cd mdm-platform && npm test:conflicts
```

- [ ] **Step 4: Commit**

```bash
git add mdm-platform/server/routes/conflicts.js mdm-platform/server/index.js
...
**Files:**
- Modify: `mdm-platform/public/index.html` (complete rewrite)
...
**Files:**
- Modify: `mdm-platform/public/index.html` (add JS modules in `<script>`)
...
**Files:**
- Modify: `mdm-platform/public/index.html` (replace login/data loading code)
...
**Files:**
- Modify: `mdm-platform/public/index.html` (final integration)
