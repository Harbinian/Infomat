...
> 边界：不修改 `docs/norms/`、PMO 驾驶舱或流程输入基线。
...
- 职责引导和强权限边界分开：项目治理角色提示人该做什么，权限点决定系统是否放行。
- 每个人必须至少有一个基础权限角色，项目治理角色只能叠加，不能单独存在。
...
岗位和行政等级只做校验、提示和待确认排序，不作为最终响应责任人的唯一来源。
