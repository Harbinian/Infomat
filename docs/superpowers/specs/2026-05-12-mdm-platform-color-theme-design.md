...
**日期**: 2026-05-12  
**来源**: 从 `mdm-platform/logo.png` 提取配色（红 #fc0000 47.7% + 黑 #000000 32.9%），重构前端 CSS 变量以建立品牌一致性。
...
### Topbar
- 背景改为 `var(--header-bg)`
- 左侧新增 logo 图片：将 `mdm-platform/logo.png` 复制到 `mdm-platform/public/logo.png`，HTML 中引用 `/logo.png`，约 40px 高
...
仅 `mdm-platform/public/index.html`（单文件应用）。
