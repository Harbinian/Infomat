...
- 从 L3/A1 查看相关字段台账和黄金源候选。
- 字段台账可引用流程治理快照中的 L3 或 A1。
- 黄金源候选系统仍按字段治理逻辑维护，不因流程建议落位自动认定。
- 冲突管理可以识别同一字段在不同部门、不同流程、不同候选系统中的不一致。
...
- 主数据域蓝图：组织、人员、岗位、物料、供应商、客户、项目、产品、工装、设备、质量记录等候选域。
- 黄金源蓝图：每类主数据的候选来源、维护部门、消费系统、同步方式和治理责任。
- 接口蓝图：MDM 与 ERP、MES、OA、PLM、集成总线的候选接口清单。
