...
| 字段清单 | 支撑后续数据地图、黄金源和主数据候选 |
| 提交说明 | 说明为什么新增、影响哪些部门、是否需要跨部门确认 |

没有来源文件的新增流程可以先作为草稿或候选保存，但应标为“待部门确认 / 待补证据”，不能直接进入正式流程地图。
...
| MDM-L3-01-A06 | 复核主数据对象候选和黄金源 | 数据质量员 / 审核员 | 数据对象、关键字段、消费系统、维护部门 | 主数据候选、黄金源确认意见 | `流程治理 -> 证据来源`、`数据质量` |
...
| MDM-L3-02-A06 | 发布或纳入回源整改 | 管理员 / 信息化负责人 | 已通过的新增流程 | 发布记录，或待回写的正式映射变更 | `报送管理`、仓库正式映射文件 |
...
| 看证据是否够 | `流程治理 -> 证据来源` | 源文件覆盖、主数据对象候选、证据链 | 不直接改；记录缺少的文件、条款、页码或表单 |
...
这条线处理“现有流程治理快照里没有，但部门认为确实存在”的流程。它的目标不是绕过 `docs/norms/`，而是先在 MDM 形成草稿、字段和审核记录，再由信息化工作组决定是否回写正式映射。
...
| 主数据候选 | 只看到字段相似，未确认维护部门和黄金源 | 维护部门、审批部门、消费场景和黄金源候选已记录 |
