...
| 文档结构化输出 | Document Structured Output | MDM 流程治理中的制度创建能力，把制度说明（含制度编号、制度名称、拟发布版次、当前有效版次、L1 能力域、L2 业务能力、目的、范围和与已有制度/流程/表单的关系）、术语、流程与行为、跨部门承接、附表结构、证据、结构化预览和 Markdown 草案保存为可审核、可发布的结构化草稿；制度编号全公司唯一，版次由系统按 A、B、...、Z、AA 自动生成；涉及其他部门从平台部门清单多选，`全公司` 表示包含所有部门但不是一个真实部门；制度说明中的 L1 能力域和 L2 业务能力从 MySQL 中本部门已有映射关系选择，流程与行为默认沿用，不在此处新增；标准数据合同见 `docs/contracts/document-structured-output.schema.json` |
