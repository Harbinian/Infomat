...
| 三级管理 | Three-Level Management | 本项目人员管理口径：公司领导层（总经理、副总经理）、部门、办公室或班组；领导层内部的分管关系不另增加一级，不从职务名称自动计算员工级别 |
| 虚拟办公室或班组 | Virtual Office or Team | 后续由项目负责人根据流程设置的部门内协作单位，需确认父部门、用途、人员归属及其与真实建制的区别；当前缺项不自动补建，也不直接产生组织编码或权限 |
