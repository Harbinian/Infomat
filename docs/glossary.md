...
| 数据对象 | Data Object | 流程中具有完整业务含义，能够与字段和具体实例区分，并会被创建、更新、使用、传递或保管的一组业务信息；它是流程编制和数据分析的基础单位，不等同数据库表，也不自动等同主数据 |
| 主数据 | Master Data | 经有权治理主体认定，用于稳定识别企业核心业务实体，并需要在多个业务行为、流程、部门或系统之间保持统一的部分数据对象；所有主数据都是数据对象，但不是所有数据对象都是主数据 |
| 表单与主数据关系 | Form and Master Data Relationship | 空白表单模板通常属于受控模板、制度附件或参考数据，填写完成的表单通常属于交易记录或业务证据；表单字段可以引用主数据，但整张表单不因此自动成为主数据 |
| 主数据认定提示 | Master Data Identification Hint | 3001前端依据稳定实体、稳定标识、复用范围、责任、来源和交易记录特征生成的非阻断提示；提示用于补充对象事实或列入交接待定事项，不代表数据对象已经被认定为主数据 |
| 七步治理导航 | Seven-Step Governance Navigation | 3001按“开始与文件、流程边界、流程骨架、动作与异常、数据与表单、跨部门核对、评审与交接”组织单流程编制和核对的页面主线；它只组织现有v6内容，不新增结构字段或审核状态 |
| 阶段草稿 | Stage Draft | 编制人在第一至第四轮主动下载的未审核v6 JSON，阶段分别为流程骨架、动作与异常、数据与表单、跨部门核对；阶段下载是无状态编制的人工留存点，不是审核节点 |
...
| 结构摘要 | Schema Digest | 对指定版本结构规则计算的SHA-256。3001当前版本为v6；MDM-AI助手固定使用v5，并在浏览器首次进入、定时检查和每次模型调用前后核对v5结构摘要，变化时返回`VERSION_CHANGED` |
...
| 旧跨部门记录 | Legacy Cross-Department Record | v1至v4文件中`cross_department_handoffs[]`的只读迁移副本。能够唯一判断时转换为业务行为、普通流程关系和数据行为关系；不能唯一判断时保留原值和原因。v5和v6使用`migration.legacy_cross_department_records[]`保存转换记录，该数组不参与当前流程计算、评分或图形生成 |
