...
| 单流程结构化文件规则 | Process Governance Structure Rules | 3001导出的`process-governance-v7`文件必须遵守的结构规则；一份JSON只包含一个`process`，记录编制元数据、普通行为与控制节点、流程关系、数据行为关系、对象字段、表单字段引用、对象级生命周期、表单、术语和只读迁移归档，不包含图坐标、可信审核状态、审核意见或批准标记。v1至v6只作为兼容迁移输入 |
| 表单与主数据关系 | Form and Master Data Relationship | 空白表单模板通常属于受控模板、制度附件或参考数据，填写完成的表单通常属于交易记录或业务证据；表单字段可以引用主数据，但整张表单不因此自动成为主数据 |
| 七步治理导航 | Seven-Step Governance Navigation | 3001按“JSON基本信息、流程边界、流程骨架、动作与异常、数据与表单、跨部门核对、评审与交接”组织单流程编制和核对的页面主线；步骤标签和当前步骤明细使用同一组页面内存提示，点击待补充项返回唯一编辑位置；它不新增结构字段或审核状态 |
| 阶段草稿 | Stage Draft | 编制人在第一至第四轮主动下载的未审核v7 JSON，阶段分别为流程骨架、动作与异常、数据与表单、跨部门核对；阶段下载是无状态编制的人工留存点，不是审核节点 |
| 未应用修改 | Unapplied Changes | 用户已经在3001的显式应用编辑器中输入，但尚未点击应用的页面内内容；该内容尚未进入当前JSON，切换编辑上下文前必须由用户选择应用、放弃或继续编辑 |
| 未下载修改 | Undownloaded Changes | 已经进入3001当前JSON，但当前JSON与最近一次成功导入或成功下载基线不同的内容；用户需要下载JSON后才能取得可恢复文件 |
| 当前内容已下载 | Current Content Downloaded | 3001当前JSON与最近一次成功下载文件的实际内容一致，且页面没有未应用修改；该状态只说明存在可恢复文件，不表示流程事实已经确认或审核通过 |
| 当前内容与导入文件一致 | Current Content Matches Imported File | 3001本次会话导入的原生v7文件与当前JSON一致，且页面没有未应用修改；该状态不表示本次会话已经成功下载文件 |
| 尚未下载 | Not Yet Downloaded | 3001当前流程为新建内容，本次会话尚未成功下载可恢复文件；即使当前JSON没有未下载修改，也不得显示为“当前内容已下载” |
| 字段补丁 | Field Patch | 属性编辑器只提交自己允许维护且由用户实际修改的字段；应用时把这些字段合并到当前最新对象，不使用完整旧对象覆盖其他步骤已经写入的内容 |
| 编辑会话 | Edit Session | 3001在当前页面内存中登记的一次显式应用编辑过程，绑定当前候选、编辑器种类和对象稳定标识；它不是服务端草稿，也不会在刷新或关闭后恢复 |
| 使用中验证 | In-Use Validation | v7软件已经发布，真实编制人、业务核对人和MDM工作组在实际使用中继续发现并记录问题的阶段；它不表示具体流程事实已经通过业务验收，也不允许把未审核v7文件直接导入3000 |
| 文件交接摘要 | File Handoff Digest | 3001按下载JSON实际UTF-8字节计算的SHA-256及对应文件名、时间、阶段和结构版本，用于交出人与接收人核对文件是否一致；摘要只在当前页面内存显示，不写入JSON，也不代表内容已经审核 |
...
| V7预览核对案例 | V7 Preview Review Case | 3000中用于持续记录一个`process-governance-v7`流程的上传修订、跨部门核对项、双方结论、依据和处理进度的预览对象。它不属于正式流程版本，不能发布，也不会把核对结果写回3001或V7文件 |
| V7受控提升 | V7 Controlled Promotion | 3000在预览核对完成后，把用户明确选择的当前修订和内容摘要写入现有正式主档与草稿链路的受控动作。它不得把V7转换为V3，不得按名称猜测主档，也不等于审核或发布；重复提升同一修订必须返回同一结果 |
| 原生V7正式版本 | Native V7 Formal Version | 3000在受控提升、正式审核和发布后形成的不可变`process-governance-v7`版本。完整V7正文、来源修订号和内容摘要保存在正式版本链中，发布返回`process_version_id`；旧L1、L2、L3和V3投影不得用假值补齐 |
...
| 对象字段 | Data Object Field | 3001在一个数据对象下定义的可复用字段，统一记录字段名称、类型和业务定义。对象字段只在当前单流程文件内有效，不等于企业级字段身份、黄金源或主数据认定 |
| 对象字段引用 | Data Field Reference | 表单主表或明细表中的字段位置对对象字段的技术引用。新建引用时，3001可以为尚未填写的表单显示名称带入对象字段名称，并按对象字段同步数据类型；表单字段仍单独保存位置、显示名称、必填、填写说明和顺序 |
| 表单显示名称 | Form Display Name | 实际纸质表单或电子表单中显示的字段名称，保存在表单字段位置的`item_name`中。它可以与所引用的对象字段名称不同；对象字段改名不得自动覆盖已经填写的表单显示名称 |
| 字段值使用方式 | Field Value Usage Mode | 表单使用对象字段值的方式，区分权威录入、引用已有值、计算生成、外部来源和待确认。该方式记录设计事实，实际自动带入、只读和权限控制由承载表单的业务系统实现 |
...
| MDM-AI助手 | MDM AI Assistant | 当前五账号试点的页面名称；该助手集中部署在内网主机，以连续对话帮助用户依据3001现行结构梳理流程，右侧同步显示结构化结果，材料只作为可选补充。AI可以发现信息缺口、前后矛盾、字段归位不清和对象混写，但不判断流程内容，也不自动写入3001或MDM平台。以后可以随功能范围调整显示名称，内部目录和启动命令保持稳定 |
| Infomat试点版本 | Infomat Pilot Version | 同时提供MDM-AI助手和3001的已提交Git版本；正式启动要求服务器工作区干净，用户电脑不保存版本副本。浏览器通过Git提交和结构摘要确认当前页面与服务器一致 |
| 结构摘要 | Schema Digest | 对指定版本结构规则计算的SHA-256。3001当前版本为v7；MDM-AI助手固定使用v5，并在浏览器首次进入、定时检查和每次模型调用前后核对v5结构摘要，变化时返回`VERSION_CHANGED` |
| 独立结构预审 | Independent Structural Review | 使用新的页面上下文，只依据待审单流程JSON和对应版本结构规则检查必需结构、类型、枚举、引用、字段归位和对象拆分；不读取填报对话，不判断流程内容。硬性结构错误必须修改，结构建议可以保持原值但必须记录理由，预审意见和处理记录不写入JSON |
| 编制参考材料 | Compilation Reference Material | `process-governance-v7.migration.reference_materials[]`中的历史兼容内容，用于说明流程编制时曾参考的制度、表单、操作说明、会议或访谈等材料，不等同正式制度关联或逐步骤证据。3001只读保留该归档；新建流程导出空数组，迁移时不参与评分、流程计算或绘图 |
...
| 判断节点 | Decision Step | 流程中只负责根据办理结果选择后续路线的控制节点，不是业务行为，也不承担校对、审核、核查、审批或批准等实际办理。3001当前结构使用`behaviors[].node_type=decision`记录判断节点，这是V7沿用的技术集合名称，不改变其控制节点语义。实际办理使用`action`，办理后再进入判断节点。判断节点名称应写成可回答的问题，至少有两个互斥且覆盖全部结果的明确出口；普通行为发出的条件线不能代替判断节点。判断节点不绑定正式执行岗位，也不参加业务行为评分 |
| 判断分支 | Conditional Flow | 从判断节点发出、根据判断结果进入后续办理步骤的条件流向关系。它与流程内部回路的区别是：判断分支继续往下办理，流程内部回路退回前序步骤重新处理。`process-governance-v7`使用`flow_relations[].relation_type=condition`记录条件和目标行为；历史文档结构化输出使用`step_transitions[]`记录条件、目标步骤和证据引用。目标为空表示流向仍待补充 |
| 流程内部回路 | Internal Process Loop | 本流程内在明确触发条件下，从当前节点返回已经存在的前序业务行为或判断节点的关系；`process-governance-v7`使用`flow_relations[].relation_type=loop`记录。回路不是独立节点，也不要求固定创建在判断节点之后；审批不通过时退回前序行为，通常是判断节点的一条回路出口 |
...
| 工作角色 | Work Role | 行政人事部受控维护的稳定业务责任分类，不直接等于人员、岗位或RBAC权限。3000目标设计区分流程或业务行为型工作角色与场景型工作角色；当前受控目录和流程结构只承接前者，场景型角色需要在目录规则、生成器和校验规则扩展后才能正式发布 |
| 工作角色绑定 | Work Role Binding | 流程治理中，业务行为与流程或业务行为型工作角色之间的一对一绑定；每个业务行为最多一个工作角色，一个流程通过不同业务行为可以包含多个工作角色。绑定必须保存业务行为标识、职责、全称和可选正式编码，不能只保存“申请人、审核人、批准人”等孤立名称；判断节点不建立工作角色绑定。该一对一规则不适用于场景型工作角色的场景任命 |
| 场景型工作角色 | Scenario Work Role | 3000目标设计中按业务场景、治理案例、能力域或改进事项任命的正式工作角色类型。场景任命分别限定被任命人、对象、动作、数据范围、有效期和退出条件；场景型工作角色不占用具体业务行为的一对一工作角色绑定，也不直接授予RBAC权限 |
| 纬理 | Facilitator | 已确认的场景型工作角色中文名称。纬理在已经授权的业务场景内承担前期规划、计划编排、业务动作推进、问题分析、任务派发、结果验收记录和持续改善；纬理不因名称或任职取得部门管理权、业务决定权、治理发布权或全局数据访问权。正式工作角色目录、场景任命和系统权限尚未发布；稳定机器语义键和场景任命类型使用`facilitator` |
| 纬理场景任命 | Facilitator Scenario Assignment | 3000目标设计中，某一人员在限定业务场景内承担纬理责任的受控任命。稳定任命保存不可变被任命人，职责、动作、对象范围、数据范围、有效期，以及验收准备、验收记录和受托验收状态变更的能力上限等内容以不可变修订保存；具体工作项是否允许受托验收状态变更仍由业务结果责任主体另行精确委托。更换人员时关闭原任命并建立新任命。任命只有在有权主体作出任命决定、本人接受、主要行政责任主体确认资源、各受影响组织分别确认本组织范围且服务端权限校验通过后才能生效。必需确认被撤回时整个任命立即停止，不保留部分权限；暂停可以按已确认条件恢复，撤销、到期、永久失效、场景关闭和被替换属于终止结果。上述内容是已确认设计方向，尚未取得3000实现、迁移和权限验证证据 |
| 任命授权版本 | Assignment Authorization Epoch | `scenario_assignment.authorization_epoch`保存的单调递增授权版本。场景任命每次生效、恢复、停止、撤销、失效或切换修订时，3000目标设计要求递增该值；业务写入必须同时校验稳定任命、当前修订和授权版本，防止旧页面或旧请求在任命状态变化后继续提交。该字段属于已确认设计方向，尚未取得3000数据库、接口和并发验证证据 |
| 验收记录权 | Acceptance Record Authority | 3000目标设计中，纬理查看已冻结验收标准和当前提交、整理成果与证据、记录逐项核对结果，或者代录具名业务验收主体已经形成决定的受限能力。该能力默认使用`record_only`，不允许纬理仅凭记录动作自行把工作项改为通过或正式退回，也不转移最终业务责任 |
| 候选验收记录 | Acceptance Record Candidate | 3000 D-06C待确认设计中，纬理在`record_only`下针对一个不可变验收冻结包提交的完整逐项核对或具名决定代录记录。系统可以计算候选总体结论，但该记录不是业务决定，也不是有效验收结果；初次启用时，只有业务结果责任主体精确采用，或者具名决定人本人登录并精确确认冻结内容后，才可能形成状态效果 |
| 有效验收结果 | Effective Acceptance Result | 3000 D-06C待确认设计中，一轮验收可以改变工作项状态的唯一只追加结果。初次启用时，该结果只能由有权主体采用候选记录、本人直接决定或作出有效受托决定后建立，并必须与状态效果、工作项事件和outbox在同一事务中提交；纬理代录本身、`in_review`和`stale`都不是有效验收结果 |
| 正式验收要求 | Formal Acceptance Requirement | 3000 D-06C待确认设计中的版本化工作类型规则。所有会形成或改变业务事实、领域对象、正式版本、权限、冲突决定、发布结果或下游状态效果的工作项必须设置`acceptance_required=true`；只有不形成业务决定、不改变领域对象且不作为下游依据的通知或客观确认事项才可由策略固定为`false`，人员不能按单项工作自由关闭验收 |
| 下游影响上下文 | Downstream Effect Context | 3000 D-06C待确认设计中，针对一个工作项版本化保存的父子、汇总、依赖、关闭门槛和自动动作传递闭包。方案、提交、验收资格、有效结果及不需验收事项的客观直接关闭都必须绑定当前修订和摘要；关系变化必须原子更新当前头，闭包未知时不得认定为低风险或直接关闭 |
| 领域动作门槛上下文 | Domain Action Gate Context | 3000 D-06C待确认设计中，针对关闭、发布、合并、生效等一个具体领域动作版本化保存的全部活动门槛和待重新核验关系。动作请求必须绑定并锁定当前修订；存在`pending_revalidation`、未知关系、未满足活动门槛或修订变化时，系统拒绝动作，验收outbox不能替代门槛核对 |
| 直接验收主体 | Direct Acceptance Authority | 3000 D-06C待确认设计中，由业务结果责任主体本人承担，或者由其在已确认验收方案中预先指定，并在后续按当前验收责任槽位修订确定的具名验收决定人。当前主体必须绑定稳定槽位、具名`person_id`、允许结果、有效期和`decision_authority_ref`；该主体可以直接逐项核对并形成通过或退回，也可以精确采用候选验收记录，但必须通过验收职责分离校验。该主体不取得最终业务责任，不能再次转委托；主体变化必须受控转派并新建验收轮次 |
| 受托验收决定权 | Delegated Acceptance Decision Authority | 业务结果责任主体针对精确工作项、验收标准修订、对象范围、验收责任槽位、允许结果和有效期，另行授予纬理按不可变标准决定通过或退回的受限能力。验收方案使用`delegated_state_transition`时，该决定才可能改变工作项状态；该能力不允许纬理修改标准、批准例外、扩大范围或取得最终业务责任。D-06A已确认该分层，D-06B已确认职责分离和禁止范围；D-06C、D-06D及3000实现、迁移和权限验证仍待完成 |
| 办理参与周期 | Work Participation Cycle | 3000目标设计中，从工作项首次开始办理、正式退回后重新办理或受控重开开始，到下一次正式提交为止的稳定参与记录范围，使用`participation_cycle_id`标识。重新办理必须继承当前成果仍然采用的历史贡献血缘，不能只统计本次修改人。办理参与周期与正式提交后才建立的验收轮次不是同一对象 |
| 验收轮次 | Acceptance Round | 3000 D-06C待确认设计中，在正式提交时针对精确提交修订和验收方案修订建立的验收范围，使用`acceptance_round_id`标识。轮次保存唯一当前核对修订指针；转派、重新提交或冻结上下文变化不复用旧轮次 |
| 成果参与记录 | Work Item Participation Record | 3000目标设计按办理参与周期只追加保存的人员参与事实，覆盖办理、提交、代理提交、共同编制、成果修改、证据处理、标准起草和标准修改等动作。正式提交时，系统按稳定`person_id`和当前提交实际引用的父子成果、证据及依赖关系，冻结直接和传递贡献人员快照；账号、角色、任命或责任槽位变化不能删除历史参与事实。参与记录不完整时，受托验收资格为`eligibility_unproven` |
| 验收职责分离策略 | Acceptance Segregation-of-Duties Policy | 3000目标设计中约束正式验收人与成果参与人必须分离的不可变版本化策略。策略先检查不可覆盖的`mandatory_deny`，再检查封闭的`low_risk_allow`及全部低风险条件；正式验收不提供自验、管理员覆盖或紧急放行例外。固定禁止规则沿父子、汇总、依赖、关闭门槛和自动动作链传播。D-06B已确认该方向，策略目录、机器代码、迁移和服务端验证尚未实现 |
...
| 旧跨部门记录 | Legacy Cross-Department Record | v1至v4文件中`cross_department_handoffs[]`的只读迁移副本。能够唯一判断时转换为业务行为、普通流程关系和数据行为关系；不能唯一判断时保留原值和原因。v5至v7使用`migration.legacy_cross_department_records[]`保存转换记录，该数组不参与当前流程计算、评分或图形生成 |
...
| 流程标准评审准备 | Process Review Readiness | 3001按固定版本`process-review-readiness-v7`对当前页面中的单流程内容执行六方面临时检查。第三方面检查业务行为逐动作可执行性；第五方面检查信息类型、创建更新使用关系、可用时间、来源线索、表单处理关系、字段业务数据归属和字段取值来源；第六方面检查数据生命周期与异常处理。检查结果不写入JSON，不保存部门现场确认，不形成正式评价 |
...
| 批量数据编辑 | Bulk Data Editing | 3001步骤5中从Excel或WPS主动复制粘贴数据对象、对象字段或数据行为关系的页面内存编辑方式。页面先解析、核对稳定引用并预览整批差异；任一行失败时整批不应用，成功后只产生一条撤销记录。它不上传Excel文件，也不改变v7结构 |
| 本轮自检项 | Current-Round Self-Check Item | 编制人在步骤1至5完成当前输入后主动运行页面检查得到的提示。修改草稿后结果失效；自检项不等于业务核对结论、交接检查事项或技术结构错误 |
| 数据对象 | Data Object | 流程中具有完整业务含义、能够与字段和具体实例区分，并会被创建、更新、使用、传递或保管的一组业务信息。供应商、采购订单、发票和付款申请可以是数据对象；供应商名称是字段；某一家具体供应商是对象实例 |
| 主数据 | Master Data | 经过有权治理主体认定，用于稳定识别企业核心业务实体，并需要在多个业务行为、流程、部门或系统之间保持统一的数据对象。所有主数据都是数据对象，但不是所有数据对象都是主数据；3001只记录单流程事实，不能直接完成主数据认定 |
| 主数据认定提示 | Master Data Recognition Hint | 3001前端根据对象定义、稳定标识、复用、表单和交易记录线索确定性生成的非阻断提示。提示不调用AI，不写入正式分类字段，不提供“确认是主数据”；责任部门和MDM工作组仍须在3000按跨流程事实完成认定 |
| 数据生命周期模式 | Data Lifecycle Mode | 3001在`data_objects[].lifecycle`下按对象记录本流程入口状态、既有流程关系对应的路径、路径内生命周期事件、出口状态和推导来源。普通模式用直接问题确认本流程是否改变对象、业务上能否使用和当前保管方式；只有涉及匿名处理时才继续确认能否识别。触发、范围、责任和异常属于临时开启的高级结构核对，不参加普通评分。它不另建自由状态机，不执行真实处置，也不要求当前流程补齐对象在其他流程中的全部生命周期阶段 |
| 停用 | Deactivate | 暂停数据对象的业务使用，后续允许重新启用。停用不等同作废、失效、归档或销毁 |
| 作废 | Void | 因错误或受控决定取消数据对象的业务效力，不允许直接重新启用。作废不等同期限届满导致的失效 |
| 失效 | Expire | 因期限届满或业务条件满足而结束数据对象的业务效力，不等同人为决定作废 |
| 归档 | Archive | 把数据对象或载体从在用保管转为归档保管，不改变既有业务效力结论 |
| 销毁 | Destroy | 受控结束纸质原件、电子原件或业务副本等载体保管的不可逆高风险业务动作。系统删除和数据库物理删除不自动构成销毁 |
| 不可逆匿名化 | Irreversible Anonymization | 永久消除数据对象可识别性的不可逆高风险业务动作，不等同系统删除、作废或载体销毁 |
