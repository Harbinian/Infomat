...
| 文档结构化输出 | Document Structured Output | MDM 流程治理中的制度创建能力，把制度目的、范围、承继关系、术语、多个流程、多个业务行为、跨部门承接和附表结构按节点保存为可审核、可发布的结构化草稿；流程的 L1 能力域和 L2 业务能力从已有映射关系中选择，不在此处新增；草稿或退回修改状态下支持明细编辑、删除和业务行为作废 |
