...
| 单流程结构化文件规则 | Process Governance Structure Rules | 3001导出的`process-governance-v7`文件必须遵守的结构规则；一份JSON只包含一个`process`，记录编制元数据、普通行为与控制节点、流程关系、数据行为关系、对象级生命周期、表单、术语和只读迁移归档，不包含图坐标、可信审核状态、审核意见或批准标记。v1至v6只作为兼容迁移输入 |
