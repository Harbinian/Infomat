...
> 2026-09-11用户决定：所有`docs/`文件均不再是真源。本表只记录术语和历史使用方式，不能独立证明业务事实、人员任命或流程规则。当前主线为3001单流程编制、手动上传V7到3000治理并形成正式版本，再依据治理结果生成程序文件。
...
| 办公室 | Office | 根据2026-09-15用户确认，办公室是承接工作的最小组织单元，所属部门承担流程和数据的归口管理；办公室负责人向成员分配任务。具体名称、归口和负责人由用户手工导入明确。 |
| 办公室成员关系 | Office membership | 一名人员可以同时属于多个办公室；办公室成员关系与人员所属部门分别保存，不自动授予正式流程审核或发布权限。 |
...
| 花名册转换副本 | Personnel Roster Conversion Copy | `docs/organization/花名册.md`是行政人事部发布Excel的可读转换副本，保留原路径、摘要、员工编号及来源位置；此前“仓库人员真源”地位已由2026-09-11用户决定撤销。人员事实回到用户指定的原始Excel核对，角色任命另需明确依据，副本不证明已导入3000或账号有效 |
| 三级管理 | Three-Level Management | 按2026-09-18用户确认，一般部门采用公司领导层、部门、办公室的管理表达；复材车间不设办公室，其内部按车间、工区、班组记录。不由组织路径推定员工职级或审批层数。 |
| 虚拟办公室或班组 | Virtual Office or Team | 历史合称；2026-09-18用户明确办公室全部为虚拟，表示规划已获批但尚未正式成立。该状态不自动适用于工区或班组，也不直接产生任命、组织编码或权限。 |
| 工区 | Work Area | 复材车间下、班组上的组织层；按用户指定员工基础信息表保留具体名称及班组归属，不将工区改称办公室，不合并跨工区同名班组。 |
...
| 民机OA账号 | Civil Aircraft OA Account | 最新发布花名册原列名为“民机口径OA编号”，仓库沿用“民机OA账号”列名以兼容只读入口；原值用于辅助识别，不替代员工编号，也不证明存在3000账号或登录权限 |
| 合同类别 | Contract Category | 花名册的原始分类字段，如在岗、实习、外包单位名称等；原值保留，不自动映射为 `person.status`、账号有效状态或任职有效期；同一员工编号发生冲突时需核对来源 |
| 核查状态 | Verification Status | `花名册.md` 中记录人员资料来源处理情况的字段。当前“行政人事发布”表示依据用户确认的行政人事部发布文件转换，不代表任命或账号权限有效；历史合并状态只在历史版本中解释 |
...
| 数据生命周期治理工作包 | Data Lifecycle Governance Work Package | 3000在流程发布后由MDM工作组处理的治理对象，固定绑定一个不可变`process_version_id`、来源内容摘要和候选规则版本；它保存待定候选、MDM结论、定向业务事实答复和审核记录，不复制完整流程正文，也不读取原始3001文件 |
| 定向业务事实问题 | Targeted Business Fact Request | MDM工作组因一项专业治理判断缺少流程事实时，向一个具体业务部门提出的单一问题。业务部门只回答事实并提供制度、表单或台账依据，不承担主数据认定、对象合并、关键字段判定或生命周期规则制定 |
| 台账事实核对 | Ledger Fact Check | P06实现中，MDM针对对象或字段的固定版本和明确内容范围发起定向问题，业务人员答复事实与证据后由具备结构核对权限的人员完成核对；`checked`只表示该项事实核对完成，不能作为新对象或字段的正式治理认定。相关内容变化后保留原意见，并要求针对新内容重新核对 |
| V7 固定来源 | Fixed V7 Source | P07 中分别绑定独立材料批次、预览案例及修订或已发布流程版本的来源引用，保存内容摘要及算法；没有原始上传字节时不能以重新序列化内容冒充原件摘要 |
| 设计交接关系 | Design Handoff | P08中固定两端流程行为、对象字段和映射版本，记录标识对应、转换依据与交付接收条件的设计记录；材料声明、分析待定、人工确认分别保留，不表示实际同步或接收已完成 |
| 分析运行 | Analysis Run | P09中固定输入引用、版本摘要、检查范围、解析器和规则版本的一次分析记录；重跑产生新运行，部分结果或未检出不能自动关闭问题 |
| 分析领取令牌 | Analysis Claim Token | P10 每次原子领取生成的随机令牌；结果写入同时核对令牌摘要、租约及运行终态，过期或取消后的进程不能提交迟到结果 |
| 分析租约 | Analysis Lease | P10 工作进程在限定时间内办理一个运行的资格；心跳不超过步骤硬超时，过期后保留历史并按有限重试策略恢复 |
| 确定性 V7 材料检查 | Deterministic V7 Material Check | P11 对固定 V7 来源执行版本化技术约束及有前提的图结构检查；发现保持待核实，完整执行不等于业务正确，缺少明确入口语义时不启用不可达检查 |
| 步骤尝试 | Step Attempt | 同一分析运行内某个计划步骤的一次执行记录；重试增加尝试序号，保留此前覆盖情况、发现和证据 |
| 分析发现 | Analysis Finding | 归属具体运行及步骤尝试的待核实结果，保存规则、固定主体引用、语义定位及证据；与既有正式治理问题分开，不能自动认定或关闭问题 |
| 交接修订影响 | Handoff Revision Impact | 按对象或字段稳定身份列出当前及历史关系引用；新定义不替换旧关系的固定版本，来源、定义或映射变化显式标为需重新核对 |
| V7 台账映射 | V7 Ledger Mapping | 将固定 V7 中的局部对象、字段引用明确关联到平台稳定身份及固定定义版本的修订记录。`candidate` 为待核实建议，`confirmed` 仅表示人工确认对应关系，不表示主数据认定或流程发布；来源或父映射变化须重新核对 |
| 确定性待定候选 | Deterministic Pending Candidate | 系统根据固定版本中的结构和公开规则重复计算得到的可解释建议；候选必须保持待确认，不调用AI、不自动写成MDM治理结论，也不覆盖人员已经记录的判断 |
...
| 候选验收记录 | Acceptance Record Candidate | 3000 D-06C已确认设计方向中，纬理在`record_only`下针对一个不可变验收冻结包提交的完整逐项核对或具名决定代录记录。系统可以计算候选总体结论，但该记录不是业务决定，也不是有效验收结果；初次启用时，只有当前占用验收责任槽位的业务结果责任主体或有效直接验收主体精确采用，或者具名决定人本人登录并精确确认冻结内容后，才可能形成状态效果 |
| 有效验收结果 | Effective Acceptance Result | 3000 D-06C已确认设计方向中，一轮验收可以改变工作项状态的唯一只追加结果。初次启用时，该结果只能由有权主体采用候选记录、本人直接决定或作出有效受托决定后建立，并必须与状态效果、工作项事件和outbox在同一事务中提交；纬理代录本身、`in_review`和`stale`都不是有效验收结果 |
| 正式验收要求 | Formal Acceptance Requirement | 3000 D-06C已确认设计方向中的版本化工作类型规则。所有会形成或改变业务事实、领域对象、正式版本、权限、冲突决定、发布结果或下游状态效果的工作项必须设置`acceptance_required=true`；只有不形成业务决定、不改变领域对象且不作为下游依据的通知或客观确认事项才可由策略固定为`false`，人员不能按单项工作自由关闭验收 |
| 下游影响上下文 | Downstream Effect Context | 3000 D-06C已确认设计方向中，针对一个工作项版本化保存的父子、汇总、依赖、关闭门槛和自动动作传递闭包。方案、提交、验收资格、有效结果及不需验收事项的客观直接关闭都必须绑定当前修订和摘要；关系变化必须原子更新当前头，闭包未知时不得认定为低风险或直接关闭 |
| 领域动作门槛上下文 | Domain Action Gate Context | 3000 D-06C已确认设计方向中，针对关闭、发布、合并、生效等一个具体领域动作版本化保存的全部活动门槛和待重新核验关系。动作请求必须绑定并锁定当前修订；存在`pending_revalidation`、未知关系、未满足活动门槛或修订变化时，系统拒绝动作，验收outbox不能替代门槛核对 |
| 直接验收主体 | Direct Acceptance Authority | 3000 D-06C已确认设计方向中，由业务结果责任主体本人承担，或者由其在已确认验收方案中预先指定，并在后续按当前验收责任槽位修订确定的具名验收决定人。当前主体必须绑定稳定槽位、具名`person_id`、允许结果、有效期和`decision_authority_ref`；该主体可以直接逐项核对并形成通过或退回，也可以精确采用候选验收记录，但必须通过验收职责分离校验。该主体不取得最终业务责任，不能再次转委托；主体变化必须受控转派并新建验收单元轮次，不重建仍然有效的父验收轮次 |
| 受托验收决定权 | Delegated Acceptance Decision Authority | 业务结果责任主体针对精确工作项、验收标准修订、对象范围、验收责任槽位、允许结果和有效期，另行授予纬理按不可变标准决定通过或退回的受限能力。验收方案使用`delegated_state_transition`时，该决定才可能改变工作项状态；该能力不允许纬理修改标准、批准例外、扩大范围或取得最终业务责任。D-06A已确认该分层，D-06B已确认职责分离和禁止范围，D-06C已确认冻结与原子结果规则；D-06D及3000实现、迁移和权限验证仍待完成 |
...
| 验收轮次 | Acceptance Round | 3000 D-06C已确认设计方向中，在正式提交时针对精确提交修订和单组织验收方案或跨组织验收要求修订建立的父验收范围，使用`acceptance_round_id`标识。父轮次保存验收集合、父状态、父修订和唯一当前汇总修订指针，不保存验收人员或当前核对指针；仅人员或单元授权转派不重建父轮次，方案、要求、提交或公共冻结上下文变化时父轮次整体失效 |
| 验收单元轮次 | Acceptance Unit Round | 3000 D-06D待确认设计中，父验收轮次下针对一个稳定验收单元和当前具名验收责任建立的独立核对范围。单组织事项也建立一个单元轮次；轮次冻结验收槽位及修订、具名人员、`decision_authority_ref`、允许结果、有效期和授权基线摘要，并独占其`current_review_revision_id`和比较并交换修订。核对、候选记录、采用和单元决定均绑定并校验该冻结基线；人员或单元授权转派终结旧单元轮次并在同一父轮次内建立新轮次，旧核对、候选和决定不继承，也不能按后来的当前负责人重解释旧轮次 |
...
| 跨组织验收单元 | Cross-organization Acceptance Unit | 3000 D-06D待确认设计中，按一个受影响组织的精确业务责任范围建立的独立验收范围。单元分别保存责任主体、验收责任槽位、标准、证据、目标和单元轮次；每个组织只决定本组织单元，系统不能用协调人、多数票或缺席默认替代组织决定 |
| 验收显式汇总 | Explicit Acceptance Aggregation | 3000 D-06D待确认设计中，服务端对同一冻结包的全部必需验收单元按预先冻结的规则计算总体结果。初次启用固定使用`all_required_pass`：全部必需单元通过才能总体通过，任一必需单元退回可以总体退回；缺失、未知、冲突或陈旧单元不能默认为通过 |
| 验收责任转派 | Acceptance Responsibility Reassignment | 3000 D-06D待确认设计中，由当前业务结果责任主体或领域规则指定的有权主体，把尚未决定的验收单元后续责任转给新的具名主体。转派终结旧单元轮次并建立新轮次；新主体不能继承、编辑或采用旧轮次的核对、候选记录和决定，转派也不修改期限或清除其他限制 |
| 验收结果当前效力 | Acceptance Result Current Validity | 3000 D-06D待确认设计中，某一不可变有效验收结果当前是否仍可支持工作项状态和下游领域动作的版本化状态。每个结果保存只追加效力修订，每个工作项另有唯一当前结果头，保证同一时点最多一个结果可以被消费；只有`effective`可以被消费，`under_revalidation`同步阻断新消费，重新核验确认后以`reaffirmed_after_revalidation`为修订原因恢复`effective`，`superseded`表示旧结果只保留为历史。效力修订不能更新或删除原结果、原决定人、原时间和原授权依据 |
| 验收结果消费记录 | Acceptance Result Consumption Record | 3000 D-06D待确认设计中，关闭、发布、合并、生效等领域动作在同一事务内保存的验收结果使用事实，包含消费方、动作、领域结果版本、门槛修订、可逆性、执行主体和时间。缺少消费记录时领域动作不得使用验收结果 |
| 验收纠错事项 | Acceptance Correction Case | 3000 D-06D待确认设计中，针对新证据、来源变化、录入错误、授权缺陷、错误关闭或其他可能改变正式含义的问题建立的只追加纠错范围。报告本身不改变结果；有权主体确认后，系统先停止旧结果继续作为当前和下游门槛，再建立新轮次、待重新核验及必要的领域纠正或补偿事项，旧历史不删除、不倒签 |
| 旧验收观察记录 | Legacy Acceptance Observation | 3000 D-06D待确认设计中，从旧表或旧文件按原值、原时间、原人员值和原始快照摘要保留的只读历史。状态文字、姓名、岗位或当前组织关系不能补造成正式验收；`legacy_unproven`和`legacy_record_only`记录不能生成有效验收结果，也不能满足关闭、发布、合并、生效或自动动作门槛 |
...
| 清晰检查 | Clear Inspection | 3001主流程图的100%字号视图。页面把流程开头定位到左上可视区，用户通过平移逐段核对节点和关系；它只改变当前页面的缩放和平移，不写入JSON，也不替代“查看全图”的整体结构核对 |
| 四项任务候选界面 | Compact Four-task Candidate UI | 3001用于验证降低认知负担的默认关闭显示方式，把七个内部步骤归并显示为“文件与边界”“流程、执行人、动作与异常”“数据与表单”“检查与保存”。流程骨架和动作与异常在同一任务内通过流程图及右侧浮层编辑。它不改变process-governance-v7、阶段下载、导入导出、无状态边界或职责权限；未经业务可用性验收不得在正式3001启用 |
...
本章中标为“历史候选”的编码值和模板只用于识别旧资料，不代表当前已批准规则。航空存货四段式编码的新术语来自 `docs/integration/航空存货主数据四段式编码规范_评审稿_RUN-20260902-152518.md`，当前均为待业务批准口径。

| 术语 | 英文/缩写 | 定义 |
|------|----------|------|
| 主数据管理 | MDM / Master Data Management | 对企业核心业务实体（物料、组织、人员、产品等）的编码、属性、生命周期进行统一管理的系统和方法 |
| 主数据编码 | Master Data Code | 经有权治理主体认定、用于稳定识别一类主数据对象的业务编码；具体结构由相应对象规范确定，不能用数据库技术主键替代 |
| 企业物料身份 | Enterprise Material Identity | 企业需要长期稳定识别、具有受控技术定义和明确互换边界的业务对象；名称、旧编码、分类或库存行不能单独证明身份相同 |
| 权威存货业务编码 | Authoritative Inventory Business Code | 业务人员和 U8 存货档案用于识别企业物料身份的业务编码；航空存货评审稿固定保留大类段、小类段、顺序段和扩展解释段，具体物理参数待批准 |
| 大类段 | Major Category Segment | 四段式航空存货编码的第 1 段，表达稳定的一级物料族；不得表达项目、来源、部门、状态或 BOM 角色 |
| 小类段 | Minor Category Segment | 四段式航空存货编码的第 2 段，表达大类内稳定且互斥的二级物料族；小类含义受大类约束 |
| 顺序段 | Sequence Segment | 四段式航空存货编码的第 3 段，提供不可复用的唯一序号，不承载名称、项目、时间、来源或状态等业务含义 |
| 扩展解释段 | Explanatory Extension Segment | 四段式航空存货编码的第 4 段，在同一小类内表达单一、稳定、决定身份差异且具有现场辨识价值的特征；不适用时仍保留受控中性值 |
| 类型码 | Type Code | 历史候选：旧草案使用的 3 位物料类型前缀；PRT/PRC/TLG/MTR/CTL/TLS/GAG/FST/EQP/CSM 未经本轮四段式规则批准，不得作为当前活动字典 |
| 零组件 | PRT / Part | 历史候选编码类型；设计零部件是否成立企业物料身份，应依据受控设计定义和互换边界重新判断 |
| 工艺组件 | PRC / Process Component | 历史候选编码类型；虚拟组件不因出现在工艺结构中自动成为独立存货主数据 |
| 工装 | TLG / Tooling | 历史候选编码类型；工装属于独立主对象治理范围，不因当前存货档案自动按物料编码 |
| 主要材料 | MTR / Material (Raw) | 历史候选编码类型；材料按受控规范、身份属性和互换边界重新分类 |
| 刃具 | CTL / Cutting Tool | 历史候选编码类型；是否属于存货、工具或资产对象需按真实业务边界确认 |
| 一般工具 | TLS / Tools | 历史候选编码类型；不代表已批准的当前编码段或对象归属 |
| 量具 | GAG / Gauge | 历史候选编码类型；量具对象边界和代码未由四段式航空存货评审稿批准 |
| 工装标准件 | FST / Fastener / Standard Parts | 历史候选编码类型；标准件应依据标准签发组织、完整标准和技术规格判断身份 |
| 设备 | EQP / Equipment | 历史候选编码类型；设备属于独立主对象治理范围，不因当前存货档案自动按物料编码 |
| 机物料 | CSM / Consumable / Supplies | 历史候选编码类型；制造消耗物是否成立物料身份，应依据受控产品定义、质量影响和稳定规格判断 |
| 编码模板 | Coding Template | 历史候选：项目归口型、材料规范型、标准号型和分类流水型四种旧模板；当前航空存货评审采用四段式统一语义，物理码型尚未批准 |
| 校验位 | Check Digit | 用于发现手工录入错误的附加字符；是否采用以及算法均须依据现场差错样本和系统兼容性另行决定，不得占用扩展解释段 |
| MOD11 | Modulus 11 | 历史候选校验算法；当前航空存货四段式评审稿未批准使用 MOD11 |
| 项目号 | Project Code | 表达客户、项目或机型的业务标识；属于适用关系，不得进入航空存货四段式编码 |
| 部段号 | Segment Code | 表达飞机部段或结构位置的业务标识；属于结构上下文，不得进入航空存货四段式编码 |
| 说明型编号 | Descriptive Number | 历史候选模板中的工程指定编号；不能替代受控工程定义或企业物料身份 |
| 扩展位 | Extension | 历史候选模板中的 5 位多维字段；不同于当前四段式的“扩展解释段”，不得继续承载批产/研制、交付层级和构型等多个维度 |
| 规范短码 | Spec Short Code | 历史候选映射值；完整材料规范仍应作为独立受控标识保留，短码是否使用须重新评审 |
| 型类级 | Type-Class-Grade | 历史候选的材料型、类、级压缩段；未取得真实材料规范和样本前不得作为当前编码段 |
| 标准短码 | Standard Short Code | 历史候选映射值；完整标准号仍应作为独立受控标识保留，短码是否使用须重新评审 |
| 拼装模式 | Assembly Mode | 历史候选编码生成模式；当前航空存货任务不设计或实施编码系统 |
| 映射模式 | Mapping Mode | 历史候选编码生成模式；当前航空存货任务不设计或实施编码系统 |
| 流水模式 | Sequence Mode | 历史候选编码生成模式；当前航空存货顺序段的唯一范围和实现方式均待批准 |
| scope_key | Scope Key | 历史候选的流水切分技术参数；不属于当前航空存货源规则结论 |
| 幂等请求 | Idempotent Request | 通过稳定请求标识避免重复处理同一业务请求的技术机制；具体编码申请实现不属于当前四段式源规则范围 |
| 编码流水 | Code Sequence | 提供不可复用序号的业务概念；具体存储、并发和分配实现不属于当前四段式源规则范围 |
| 短码映射 | Short Code Mapping | 将原始外部标识关联到受控短码的候选机制；原始标识必须独立保留，是否采用短码须按对象和兼容性评审 |
| 一物一码 | One Object One Code | 经核实的一个企业物料身份只分配一个权威存货业务编码，一个编码不得代表多个企业物料身份；历史映射冲突须单独核实 |
| 生命周期状态 | Lifecycle Status | 表达主数据当前可用性及其变化的独立属性；状态不得嵌入航空存货四段式编码 |
...
| 物料主表 | Material Master | 统一物料主数据对象的历史设计草案；具体字段、表结构和系统承载不属于当前航空存货四段式源规则范围 |
| 上机常温/低温/金属/胶漆 | Airborne Material Categories | 历史候选材料分类；混合了适用性、存储或技术类别，未经真实样本和业务评审不得作为当前大类或小类字典 |
...
| express-session | express-session | Express会话中间件；MDM平台会话身份只保存`personId`、`accountId`和`authVersion`，另保留Cookie及CSRF必要元数据；显示身份和有效权限按请求重新读取 |
| 进程存活 | Liveness | 3000进程能够响应`/api/health`；不代表数据库、必要结构或业务办理已经可用 |
| 业务就绪检查 | Readiness Check | 3000通过`/api/ready`限时检查运行配置、MySQL连通与必要结构，并返回源码摘要；技术就绪不等于真实业务验收 |
| 运行源码摘要 | Runtime Source Digest | 3000启动时对server、public及应用package/lock文件计算的SHA-256摘要，用于核对运行代码；Git HEAD仅作为另行记录的追溯信息 |
...
基于固定设计交接、台账与V7映射进行核对。确定缺陷（definite_defect）仅指可由固定值证明的登记矛盾，发现仍为待核实，不能代替正式问题认定；待业务核对（business_question）表示对应说明、身份、标识或确认缺失；未覆盖（not_covered）表示缺少适用依据或执行能力。handoff.coverage只保存未覆盖说明，不表示实际范围已经检查。主链框架未获用户确认时，只展示登记连接及待核实断点。

### 跨运行发现对照（P13技术术语）

发现实例ID（finding_id）标识某次尝试保存的一条发现。跨运行匹配键（match_key）由analysis-diff-v1根据规则语义、材料逻辑身份、稳定对象/字段语义标识及部门范围派生，不替换旧comparison_key。新增（added）和本轮未再检出（not_detected）只适用于检查范围可比且对应规则完整覆盖的情况；后者不代表整改或问题关闭。持续（persistent）表示身份及引用证据未变；证据变化（evidence_changed）表示同一发现的固定证据或发现类型改变。不可比较（incomparable）表示来源、规则、范围或覆盖条件不满足；待人工匹配（manual_match）表示现有稳定身份或证据不足以自动判定。上述状态均为分析结果，不是业务认定或审批状态。

## 分析发现人工确认与问题关联

分析发现人工确认是有权人员针对一个固定运行中的具体发现，保存理由、证据、明确归口、来源版本、人员和时间的动作。机器原始结果保持不变，人工记录独立追加。confirmed表示发现已确认但尚未关联问题；not_an_issue表示仅该条未关联发现被说明不是问题；linked表示发现已显式关联现有治理问题实体。这些状态不表示问题已关闭、主数据已认定或流程已发布。

问题关联修订是分析发现与现有问题实体建立关系时的并发版本；它与问题原字段摘要共同防止错误覆盖。一个问题可关联多条发现，读取时必须具备全部关联来源范围。它不是第二套问题库，也不是办公室待办状态。
# P17技术增量用语（不作为业务治理真源）

- **问题办理用途**：交办人明确选择的核实、整改、协调或复核任务用途；机器标识为verify、correct、coordinate、review，不构成新增MDM角色或正式复核授权。
- **办理轮次**：同一问题、同一用途下由交办人明确开启的连续办理次数。前轮办结后才能开启下一轮，保留前轮历史；不表示问题已关闭或重新打开。
- **逐问题复核指定**：归口部门最终负责人本人为一个问题明确指定合格复核人员并确认关闭条件；候选角色不直接授予关闭权，指定不增加材料读取权限。
- **复核上下文摘要**：本次问题修订、原固定来源及当前获准版本、关闭条件、办理修订和结果的联合摘要；提交时复核一致性，防止旧依据用于关闭。
- **复核指定暂停**：人员身份、候选资格、必要来源权限或办理分离条件不再满足时保存的失效记录；恢复资格不恢复原指定，须负责人本人重新指定。


- Excel固定证据（excel_material）：从用户明确上传的.xlsx形成的不可变读取快照，绑定原始字节摘要、解析器和显式关联。仅作为材料证据，不表示业务台账迁移、主数据认定或公式计算结果已核实；机器分析输入继续复用template批次外键。

- Word结构锚点（word-evidence-docx-v1）：固定DOCX正文中的段落、表格、行或单元格位置，以包内部件、XML路径、结构顺序和父锚点表达。只在同一原始字节摘要的固定快照中有效，不表示页码、跨文件身份或业务对应关系。
- DOCX固定证据（word_material）：用户主动上传有效.docx后形成的不可变读取快照，绑定原始字节摘要、解析器和显式关联。机器输入复用template批次外键，业务事实仍待核实；不接受.doc或自动转换结果冒充原上传字节。

## PDF固定证据读取（P18技术实现术语）

- PDF固定证据：绑定原始字节摘要和解析器版本的不可变文本读取快照，机器source_kind为pdf_material，复用template输入种类和来源batch_id；不表示模板导入或业务事实确认。
- PDF物理页序号：从1开始的文件页序号，不等同页面印刷编号。文本锚点仅在固定快照内有效，记录页号、片段序号及坐标，不承担跨文件身份。
- PDF提取不足：该页未提取到文字，不能据此判断空白或扫描件；OCR未执行。疑似乱码仅据替换字符提示，未提示不等于文字正确或完整。

本节仅记录技术语义，不作为业务治理依据。

- 离线 AI 意见校验（analysis-ai-offline-v1）：使用本机静态字符串替身验证分析接口和结构化输出。引用通过只说明固定版本、定位和摘录匹配，意见仍待核实；真实 AI 分析保持未覆盖，不构成正式工作包结论、问题关闭依据或真实模型质量证明。
