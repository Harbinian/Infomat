...
| 周会事项台账 | Weekly Issue Ledger | PMO 周会试运行入口，用于把现场形成的行动项、风险、问题、变更和责任池事项登记到对应台账，并记录关闭标准；当前是 PMO 看板浏览器本地试运行数据，不替代 PMO Markdown 真源或 MDM 正式台账 |
| 文档结构化输出 | Document Structured Output | MDM 流程治理中的制度创建能力，把制度说明（含制度编号、制度名称、拟发布版次、当前有效版次、目的、范围和与已有制度/流程/表单的关系）、术语、流程与行为、跨部门承接、附表结构、证据、结构化预览和 Markdown 草案保存为可审核、可发布的结构化草稿；制度编号全公司唯一，版次由系统按 A、B、...、Z、AA 自动生成；流程的 L1 能力域和 L2 业务能力从 MySQL 中本部门已有映射关系选择，不在此处新增；标准数据合同见 `docs/contracts/document-structured-output.schema.json` |
...
| 文档结构化输出标准 Schema | Document Structured Output Schema | 文档结构化输出的机器可读合同，统一制度草稿、制度档案、术语、L3 流程、A1 业务行为、跨部门承接、表单字段、证据、主数据需求和待确认问题字段；该合同不替代流程输入基线或组织真源 |
