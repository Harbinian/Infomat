...
| 对象字段引用 | Data Field Reference | 表单主表或明细表中的字段对对象字段的技术引用。引用复用对象字段的名称、类型和业务定义，表单仍单独保存位置、必填、填写说明和顺序 |
...
| 工作角色 | Work Role | 与具体业务行为绑定的稳定业务责任分类，名称必须保留行为语义，例如“费用审核行为的审核角色”；判断节点只是路线控制，不单独绑定工作角色。工作角色不直接等于人员、岗位或RBAC权限，一个工作角色可以由多个岗位容纳 |
| 工作角色绑定 | Work Role Binding | 业务行为与工作角色之间的一对一绑定；每个业务行为最多一个工作角色，一个流程通过不同业务行为可以包含多个工作角色。绑定必须保存业务行为标识、职责、全称和可选正式编码，不能只保存“申请人、审核人、批准人”等孤立名称；判断节点不建立工作角色绑定 |
