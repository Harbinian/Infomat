...
| 第 2 层 | `/api/org/users` 员工目录暴露面过宽 | 已确认 | 第二批小片已处理目录收窄和后端候选人接口 |
...
- 第二批继续收窄员工目录：`/api/org/users` 保持管理员专用，新增后端最小候选人接口供后续前端切换。

## 5. 后续建议顺序

1. 第二批剩余 MDM 安全边界：默认口令、业务角色过滤口径、前端用户选择入口切换到最小候选人接口。
