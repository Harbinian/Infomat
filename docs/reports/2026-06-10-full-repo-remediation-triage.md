...
| 第 3 层 | `sourceManifest` 可能与磁盘源文件脱节 | 已确认 | 第三批小片已补 `test:source-manifest-hashes` |
...
1. 第三批优先处理工程技术部真源归属：确认是否存在独立制度包或部门资料目录；对科技创新部、数字工程部、集成研发候选材料逐项确认所有者、适用范围和版本；确认前不生成工程技术部 DCM/BBM。
