...
> 日期：2026-06-11  
> 范围：只定义 `crossDept` 风险数据的新鲜度校验口径，不修改流程输入基线、生成快照或 PMO 驾驶舱。
...
这说明 `crossDept` 已经能和报告正文保持一致，但还不能回答一个更关键的问题：这份报告是否由最新的流程输入基线重新生成。
