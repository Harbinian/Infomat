...
> 适用范围：本文件是评审报告，不是流程输入基线、PMO 真源或 MDM 平台配置真源。
...
| 字段治理 | 字段台账、字段身份、待确认黄金源 | MDM `field_entries`、`field_identities` | 录入、确认、冲突、发布 |
...
- `data_object` 是主数据对象待确认，不等于已经完成主数据建模。
- 黄金源确认必须按字段进行。
...
| MDM 初始密码策略 | 平台用户管理和批量用户脚本已改为一次性初始密码并标记首登改密；历史账号是否仍使用旧固定口令需另行 dry-run 清查 | 安全评审专项 |
