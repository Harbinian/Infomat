...
> 日期：2026-06-10  
> 范围：仓库根目录文件，不含目录内容。  
> 结论：本报告先做分流；2026-06-11 已将 `analyze-layout.js` 迁入 `scripts/` 并保留根命令入口，已将 `temp_survey.txt` 迁入基础设施资料目录。
...
| `temp_survey.txt` | 06B 厂房接入民机非密园区网需求调查表抽取文本 | 已迁入 `docs/HardwareResearch/06B厂房接入民机非密园区网需求调查表_抽取文本.txt` |
...
- 已从根目录迁入 `docs/HardwareResearch/06B厂房接入民机非密园区网需求调查表_抽取文本.txt`。
- 内容是“昌兴06B厂房接入民机非密园区网”的需求调查表抽取文本，用于需求收口、方案编制、费用测算与领导汇报准备。
- 文件含联系人信息，后续引用、外发或转为正式方案前需确认脱敏和适用范围。

判断：

- 归属更接近基础设施和网络接入资料，而不是流程输入基线、组织真源或 PMO 当前计划。
- 本轮只做原样迁移，不改正文、不重排表格。
...
| P1 | 复核 `temp_survey.txt` 的资料归属 | 已处理：迁入 `docs/HardwareResearch/`，保留原始抽取文本 |
