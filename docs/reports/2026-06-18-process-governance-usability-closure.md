...
> 状态：已实施  
> 范围：MDM 流程治理承接页、候选复核导入/展示、仓库级候选报告生成脚本  
> 边界：不修改 `docs/norms/` 流程真源，不反向覆盖 PMO 驾驶舱数据
...
2. 候选复核区出现“候选类型”“来源锚点”“内部锚点Pxx”等技术口径。
...
- MDM 候选复核接口与仓储：`apps/mdm-platform/server/routes/processGovernance.js`、`apps/mdm-platform/server/processCandidateReviewRepository.js`
- MDM 流程治理 MySQL 读模型：`apps/mdm-platform/server/processGovernanceMysqlRepository.js`
- 应用内测试：`apps/mdm-platform/scripts/test-*.js`
- 仓库级候选报告脚本：`scripts/candidate-review-core.mjs`、`scripts/build-candidate-sankey-preview.mjs`
...
node scripts/test-process-candidate-review-mysql.js
node scripts/test-process-candidate-review-api.js
node scripts/test-process-governance-api.js
cd ../..
node scripts/test-candidate-review-mysql.mjs
node scripts/test-candidate-sankey-preview.mjs
```

说明：本轮没有运行 `scripts/parse-sankey-data.mjs`，因为没有修改流程真源 `docs/norms/`。
