...
| `pmo/echarts.min.js` | 与根目录同 hash，当前未发现页面引用 | 作为后续低风险删除待确认；删除前再跑引用扫描和 PMO 页面校验。 |
...
| `pmo/echarts.min.js` | 当前引用扫描未发现真实页面引用；PMO 文档也指向根目录 `../../echarts.min.js` | 后续可删除待确认。 |
...
| 阶段 | 动作 | 验证 |
|---|---|---|
| P1 | 删除待确认预检：再次扫描 `pmo/echarts.min.js` 引用；若仍为 0，删除该文件并更新文档中任何误导性描述 | `rg "pmo/echarts.min.js|src=\"echarts.min.js\"" pmo`、打开 PMO 驾驶舱或运行现有 dashboard 检查 |
