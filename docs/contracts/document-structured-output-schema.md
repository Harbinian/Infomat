...
| `work_role_bindings` | L3 负责人或 A1 参与角色与行政人事部正式工作角色的候选/确认关系；v2 可选字段 | 当前随结构化 JSON 往返，尚不写入 MDM |
...
`work_role_bindings[]` 使用以下固定字段：`binding_ref`、`process_ref`、`step_ref`、`participant_department`、`source_role_text`、`work_role_code`、`participation_type`、`status`、`evidence_refs`、`confirmation_basis`。
...
- `proposed` 只表示流程治理提出的候选关系；`confirmation_basis` 可为空，但自动抽取形成的候选仍应携带证据引用。
...
- `work_role_code` 必须来自 `docs/organization/工作角色目录与岗位映射.md` 生成的正式目录，并使用行政人事部顺序分配的 `WR-0001` 编码。文档抽取不得自行编造编码；原文角色称谓保留在 `source_role_text`。
