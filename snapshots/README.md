...
本目录保存历史 norms 快照，用于追溯某个时间点的流程映射状态。当前流程真源仍在 `docs/norms/`，当前生成快照为 `docs/company-sankey-data.json`。
...
1. 不从本目录反推当前流程数据。
2. 修改当前流程映射时，回到 `docs/norms/` 并运行 `scripts/parse-sankey-data.mjs`。
