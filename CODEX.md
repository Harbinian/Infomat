...
`codex/3001-v7`隔离候选导出`process-governance-v7`，兼容导入v1至v6、v7和历史多候选结构化JSON。流程图、数据流、数据生命周期、文字编制和属性面板共同读写一份页面内存草稿；生命周期分析使用本地确定性规则，主数据认定提示不写入正式结论。旧交接、工作角色、参考材料和内部流程调用只读归档。正式3001在完成真实脱敏流程验收和发布签发前仍使用v6；v7文件暂不能直接导入3000；MDM-AI助手仍固定使用v5。
