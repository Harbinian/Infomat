...
正式切换前，组织与正式工作角色真源在 `docs/organization/`；3000 行政人事功能完成对应首版发布并正式切换后，组织与正式工作角色以 3000 发布版本为唯一正式真源，仓库文件改为受控只读导出。
`docs/organization/花名册.md` 只作为人员参考材料，不是员工主数据真源；3000 行政人事功能完成首版发布并正式切换后，员工主数据以 3000 发布版本为唯一正式真源。
...
- `花名册.md` 是人员、工号、部门和岗位的参考资料，不是员工主数据真源，不要求与 MySQL person 或 3000 已发布员工逐条统一对账。
