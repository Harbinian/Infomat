...
- 表单与记录在独立工作区按整张纸质表单展示字段。用户逐字段确认主表或具体明细表归属、业务数据归属和取值来源；JSON继续使用`forms[].areas[].items[]`，并用`form_design_state`区分现状、拟设计和历史待确认状态，不增加页面状态或图形字段。
- 3001不保存草稿、不记录审核状态，也不通过API、数据库、队列、回调、共享会话或轮询与3000通信。
- 3001只保存流程编制内容和单文件技术引用，不保存MDM审核意见或批准标记。
- v6中跨部门由业务行为的固定执行部门与流程归口部门自动识别。跨部门先后和返回使用普通`flow_relations[]`，数据传递和返回使用`data_objects[].behavior_links[]`；活动结构不再包含独立交接对象。
...
- 3000仍按自身当前v1至v3结构规则运行，不属于3001 v6范围。3001页面必须明确v6文件不能直接导入3000。
- 3001后端保留确定性文档解析器用于历史迁移和回归测试，但页面不再提供参考材料解析入口。模型辅助填报能力已经移除；3001的启动、访问、新建、导入、校验和下载均不依赖DeepSeek或任何辅助服务。
- 3001页面按“JSON基本信息、流程边界、流程骨架、动作与异常、数据与表单、跨部门核对、评审与交接”组织现有v6内容。步骤状态和待补充项使用同一组页面内存提示，点击待补充项返回唯一编辑位置。页面提供四轮阶段下载和最终待核对下载；每次下载按最终文件实际字节计算SHA-256，摘要用于人工交接核对，不写入JSON。
