...
以下为`codex/3001-v7`隔离候选主线。正式3001在真实脱敏流程业务验收和发布签发前仍使用v6。
...
- 3001页面按“JSON基本信息、流程边界、流程骨架、动作与异常、数据与表单、跨部门核对、评审与交接”组织现有v7内容。步骤5在同一对象下提供数据流和生命周期互斥视图；步骤7按六方面核对。步骤状态和待补充项使用同一组页面内存提示，点击待补充项返回唯一编辑位置。页面提供四轮阶段下载和最终待核对下载；每次下载按最终文件实际字节计算SHA-256，摘要用于人工交接核对，不写入JSON。
