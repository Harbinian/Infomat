...
- v5中跨部门由业务行为的固定执行部门与流程归口部门自动识别。跨部门先后和返回使用普通`flow_relations[]`，数据传递和返回使用`data_objects[].behavior_links[]`；活动结构不再包含独立交接对象。
...
- 3000仍按自身当前v1至v3结构规则运行，不属于本次3001 v5升级范围。3001页面必须明确v5文件不能直接导入3000。
