...
1. **启动任何开发任务前**，根据下表 Read `docs/glossary.md` 对应章节获取语境：

| 任务涉及 | 读取术语表章节 |
|----------|-------------|
| 部门/能力/流程/系统/映射/审批/桑基图 | `## 1. 业务域` |
| 物料编码/类型码/编码引擎/产品/组织/人员 | `## 2. 主数据域` |
| GLC/GLB/GLG/体系文件/部门文件号 | `## 3. 体系文件域` |
| RBAC/权限/SCD/中间件/SQLite/路由/导入导出 | `## 4. 技术域` |
| 沈飞民机/SACC/经营/项目/报价/交付/结算 | `## 5. 供应链协同域` |
...
每次 `git commit` 前，执行以下检查（不可跳过）：

- [ ] 扫一遍本次 `git diff --cached` 中引入的新中文业务名词、新缩写、新表名/字段名
- [ ] 逐个 `grep` `docs/glossary.md` 确认已收录或无需收录（临时变量名/局部常量不纳入）
- [ ] 有任何一个未收录？→ 先按 `docs/glossary.md` 末尾的"术语新增流程"补全，再 commit
- [ ] 全部已收录或无需收录 → 正常 commit

> 术语收录标准：制度、流程的名称，核心角色，核心业务动作。临时变量名、局部常量、中间计算结果不纳入。
