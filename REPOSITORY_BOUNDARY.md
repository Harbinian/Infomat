...
2026-09-11用户明确撤销全部`docs/`文件的真源身份；本文件下方历史入口和旧派生关系只用于资产定位，不作为治理依据。3001编制未审核单流程材料，用户主动下载并上传3000，由3000治理、核对、审核并形成正式版本，之后生成程序文件。业务事实与责任来自用户指定的外部原始材料及明确业务确认；程序文件是结果，不反向定义流程。人员Excel的Markdown转换副本、组织/角色说明和旧制度同样不是真源。
...
| 人员事实 | 行政人事部最新发布的Excel及其经核对转换的`docs/organization/花名册.md` | Markdown为当前仓库人员真源；不自动导入真实MySQL、不从岗位猜任命或账号角色，不把新版未收录解释为离职 |
...
| 修改代码、脚本、接口或启动命令 | 主责目录代码及信息已变化的说明 | 行为或合同已变而相关说明仍旧 |
| 写审计、评审、稳定化报告 | `docs/reports/` | 根目录散放 |
| 写架构规则或职责说明 | 根目录边界文件、`docs/architecture/`、`docs/adr/` | `docs/norms/` |

文档同步按根 `AGENTS.md` 和 `CODEX.md` 的影响范围执行，不因内部实现修改而改写仍准确的说明。
